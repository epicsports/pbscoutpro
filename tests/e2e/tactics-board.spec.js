...
test('board edit door opens the phased editor (present mode retired)', async ({ page }) => {
  ...
  await page.waitForFunction(() => !!window.__pbtest, { timeout: 20000 });
  await page.evaluate(s => window.__pbtest.setWorkspace(s), WS);
  await page.evaluate(id => window.__pbtest.seedLayoutTactic(id, 'Edit-door E2E'), BASE_LAYOUT);

  await page.setViewportSize({ width: 1194, height: 834 });
  await page.goto('/' + `#/layout/${BASE_LAYOUT}/tactics`);

  // Seeded tactic auto-selects → the Edit door floats on the field; present-enter is gone.
  const editEnter = page.getByTestId('tactics-edit-enter');
  await expect(editEnter).toBeVisible({ timeout: 20000 });
  await expect(page.getByTestId('tactics-present-enter')).toHaveCount(0);

  await editEnter.click();
  await expect.poll(() => page.url(), { timeout: 10000 }).toContain(`/layout/${BASE_LAYOUT}/tactic-edit/`);
