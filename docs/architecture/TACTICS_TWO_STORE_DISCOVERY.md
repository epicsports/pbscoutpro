...
**OP1 SUPERSEDED — full purge 2026-06-17 (Jacek GO).** When asked for OP1's target-layout rule, Jacek decided the data isn't needed: "wyrzucić wszystkie taktyki / nie potrzebujemy tych danych." Census showed all 35 remaining tactics (26 overlay + 9 tournament) live in `ranger1996` only (zero customer docs), so `scripts/migration/tactics_purge_all.cjs --live` backed up + deleted ALL 35 → **0 tactics**. No migration, no code change (feature intact, stores empty). DEPLOY_LOG 2026-06-17.

**Residual (optional Tier-1, low priority):** the dual-store CODE (two stores/hooks/CRUD + two TacticPage routes) still exists, now feeding empty stores. Harmless; a future writer-layer unify (Option B) would remove the debt if/when tactics get real use again.
