...
3. **Phase 2.3 — Teams global migration** — Hoist workspace teams to `/teams/`. Per § 63.15.2 + § 63.15.2.X (locked 2026-05-20). Sub-tasks:
   - ✅ **2.3.a — Bootstrap (DONE 2026-05-20, `a8cb308` + execute run)** — 132 workspace teams → 132 global `/teams/` docs. Option α (preserve workspace docIds as global IDs). Two-pass execution: 125 parents written first, 7 children second with parent-existence verification (0 orphans). NO automatic dedup — externalId duplicates and intra-workspace name overlaps migrate as separate global docs (per § 63.15.2.X locked policy). Verbatim schema hoist (production fields: name, leagues[], divisions{} map per Phase 2.1b name-string precedent, parentTeamId, externalId) + 3 migration tracking fields (originWorkspace, migratedAt, createdAt/updatedAt preserved). Forward-looking § 63.15.2 spec fields (pbliTeamId, leagueId, divisionId, brandId, shortName, active, createdBy/createdByWorkspace) deferred to post-Phase-2 reconciliation. Known anomaly: 1 externalId dup group (RANGER vs Ranger Warsaw) — admin curates via 2.3.c. Legacy /workspaces/ranger1996/teams/ untouched. Idempotency verified post-execute. Reports in scripts/migration/reports/.
   - ✅ **2.3.b — Workspace consumption refactor + dual-write (DONE 2026-05-20, `97af95a`, sequenced rules+code deploy)** — useTeams hook refactored in src/hooks/useFirestore.js to read /teams/ via onSnapshot, returns { teams, teamsById, loading, error } (teamsById map provides O(1) parentTeamId resolution). NO helper hooks added (useTeam/useChildTeams deferred — teamsById[id] covers the use case cleanly without wrapper indirection; matches Phase 2.2.b usePlayers minimalism). dataService addTeam/updateTeam dual-write to /teams/ + legacy /workspaces/{slug}/teams/ paths; deleteTeam stays workspace-only (global delete deferred to 2.3.c for parentTeamId children + externalId dup safety). firestore.rules /teams/ block added (read auth, create+update auth, delete admin-email). All 20 React consumers (TeamsPage, TeamDetailPage, MatchPage, ScoutedTeamPage, MainPage, MembersPage, PlayersPage, UserDetailPage, TrainingSetupPage, PlayerPerformanceTrackerPage, ProfilePage, PlayerStatsPage, NewTournamentModal, TournamentPicker, AttendeesEditor, ViewAsPlayerPicker, LinkProfileModal, CoachTabContent, ScoutTabContent, TrainingMoreTab, usePPTIdentity) auto-picked-up via centralized hook — zero consumer file changes (same Phase 2.2.b setup). breakoutVariants subcollection at /workspaces/{slug}/teams/{tid}/breakoutVariants/ unchanged per § 63.15.2 workspace-specific scouting context decision; legacy dual-write preserves parent doc so subcollection reads keep working. subscribeTeams dataService fn now orphan (deferred cleanup to 2.3.d). Sentry on hook fetch error.
   - ✅ **2.3.c — Super admin UI for teams CRUD + sister team picker + duplicate resolution (DONE 2026-05-20, `6638c54`)** — `/admin/teams` route + 5 new components (AdminTeamsPage with 8 filter pills + duplicate banner + paginated list; TeamFormModal with Identity + Sister team relationship + collapsed Audit; TeamPickerModal search-and-select with cycle prevention; TeamDuplicateResolutionView side-by-side with weighted recommendation heuristic + dynamic safety note; ChildrenOrphanWarning 3-option radio for retire). 3 new dataService fns (retireTeam/unretireTeam/setParentTeam) with cycle detection helper. useActiveTeams asymmetric hook (teams=active filtered, teamsById=all preserved) deployed alongside — 21 React consumers refactored mechanically. Schema additions all nullable additive (retiredAt/retiredBy/retirementReason/canonicalReplacementId). Soft delete via retiredAt — NO hard delete (preserves audit + reference resolution). Reference re-pointing checkbox UI shown DISABLED, deferred to 2.3.d per § 63.15.2.X.1. Defense in depth (3 layers): AdminGuard + component check + firestore.rules from 2.3.b. § 63.15.2.X.1 doc patch landed in same commit locking mockup-reviewed UX patterns. **🎉 Phase 2 Step 3 effectively COMPLETE — only 2.3.d cleanup remains.**
   - **2.3.d — Legacy /workspaces/{slug}/teams/ cleanup** — DEFERRED until 2.3.c stable + Phase 2.4 TeamMemberships maturity. Migration script to remove workspace write paths from dataService + drop subcollection. Optional: implement reference re-pointing (tournament/player teamId → canonical) for retired teams via collectionGroup updates.
