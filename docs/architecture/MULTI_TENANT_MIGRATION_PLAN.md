...
2. **Phase 2.2 — Players global migration** — Hoist workspace players to `/players/`. Dedup via `pbliId`. Update workspace UI to query global. Reference § 63.15.3.
   ...
   - **2.2.c — Super admin UI for players CRUD** — follows 2.2.b stable. Pattern reuses Phase 2.1c (admin route + AdminGuard + CRUD service fns; admin-only delete gate already in firestore.rules from 2.2.b).
   - **2.2.d — Legacy /workspaces/{slug}/players/ cleanup** — DEFERRED until 2.2.c stable. Migration script to remove legacy write from dataService + drop subcollection after stabilization window.
