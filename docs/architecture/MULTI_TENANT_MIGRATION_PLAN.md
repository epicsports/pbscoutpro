...
3. **Phase 2.3 — Teams global migration** — Hoist workspace teams to `/teams/`. Per § 63.15.2 + § 63.15.2.X (locked 2026-05-20). Sub-tasks:
   ...
   - **2.3.c — Super admin UI for teams CRUD** — Pattern reuses Phase 2.1c Leagues + Phase 2.2.c Players. Per § 63.15.2.X #4 MUST cover BOTH manual create (default for non-NXL tenants) AND PBLeagues-imported edit + sister team designation as first-class workflows. Delete safety: warn on parent (would orphan children) + warn if any player.teamId or future TeamMembership references.
   - **2.3.d — Legacy /workspaces/{slug}/teams/ cleanup** — DEFERRED until 2.3.b stable + 2.3.c admin curation complete.
