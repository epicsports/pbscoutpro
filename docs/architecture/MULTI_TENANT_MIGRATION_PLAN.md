...
3. **Phase 2.3 — Teams global migration** — Hoist workspace teams to `/teams/`. Per § 63.15.2 + § 63.15.2.X (locked 2026-05-20). Sub-tasks:
   ...
   - **2.3.b — Workspace consumption refactor + dual-write** — useTeams hook reads /teams/ via onSnapshot + teamsById map (O(1) lookup) + useTeam(id) helper for parent resolution. dataService addTeam/updateTeam/deleteTeam dual-write. firestore.rules /teams/ block. Most consumers (TeamsPage, TeamDetailPage, NewTournamentModal, ScoutTabContent, AttendeesEditor, usePPTIdentity, CSVImport, ScheduleCSVImport, ScheduleImport) use centralized useTeams hook — changes localize to hook + dataService + rules. breakoutVariants subcollection stays workspace-scoped per § 63.15.2 spec for workspace-specific scouting context.
