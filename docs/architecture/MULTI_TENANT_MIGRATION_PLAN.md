...
2. **Phase 2.2 — Players global migration** — Hoist workspace players to `/players/`. Dedup via `pbliId`. Update workspace UI to query global. Reference § 63.15.3.
   ...
   - **2.2.b — Workspace consumption refactor (usePlayers hook + alias resolution)** — pre-req: Firestore rules for `/players/`. Refactor consumers (PlayerEditModal, KIOSK, TeamDetailPage, MatchPage point.assignments resolver, etc.). Alias map for 42 canonical/alias pairs so `point.assignments[]` references to alias IDs resolve to canonical.
   - **2.2.c — Super admin UI for players CRUD** — follows 2.2.b stable. Pattern reuses Phase 2.1c.
   - **2.2.d — Legacy /workspaces/{slug}/players/ cleanup** — DEFERRED until 2.2.b stable. Migration script to delete subcollection.
