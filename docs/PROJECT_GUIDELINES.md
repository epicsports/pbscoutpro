...
- ❌ **NIE** rób `orderBy('pole')` na kolekcji, której dokumenty mogą NIE mieć tego pola — Firestore **po cichu wyklucza** dokumenty bez pola sortującego (query zwraca je jako brakujące, nie błąd). Skutek: dokument-sierota staje się niewidoczny na zawsze → ekran-detalu wisi w wiecznym „Loading…". Jeśli pole nie jest gwarantowane na 100% (np. seed/migracja go pomija), **sortuj po stronie klienta** (`getDocs` bez `orderBy` + `.sort(...)`). Przykład/regresja: scouted-team eternal-loading (2026-06-11, `subscribeScoutedTeams`).
...
- ❌ **NIE** filtruj zawodników po drużynie przez `player.teamId === X` — gracz może być w wielu drużynach (`player.teams[]`, § 72). Roster-read = `playerOnTeam(player, X)` z `src/utils/playerTeams.js` (ma on-read fallback na stary `teamId`). `teamId` zostaje tylko jako PRIMARY (nagłówek/display).
- ❌ **NIE** używaj raw `useState` setter dla stanu gestu czytanego przez `touchHandler` przez `ref` — touchHandler czyta `draggingRef.current` / `draggingBunkerRef.current` w handleMove/handleUp (poza React render cycle), więc setter MUSI być wrapped (`setDragging = (v) => { draggingRef.current = v; _setDragging(v); }`) żeby aktualizować **ref + state**. Raw setter aktualizuje tylko state — ref zamarza na null → **ciche tap/drag death** (tap-elementu nie otwiera toolbara, drag-elementu nie rusza, pan wygrywa to co miało być dragiem). Realny bug — `BaseCanvas:172-177` (Step #2 extraction `ecc850ce`) zgubił wrap z `FieldCanvas:81-86`, dormant dopóki Step #4 nie zmigrował konsumentów na BaseCanvas; naprawione 2026-05-24 (`6f7158f7`).
- ❌ **NIE** zapomnij `editable: true` w `touchHandlerState` przy nowej canvas surface przez BaseCanvas. `touchHandler.handleDown:228` + `handleMove:465` + `handleUp:466` early-return na `!editable && !layoutEditMode`. Pan/zoom dalej działają (arbiter przed editable gate), ALE tap-place / drag / element-menu / wszystko inne = ciche no-op. Każda interactive canvas surface (gdzie user STAWIA / EDYTUJE) MUSI pass `editable: true`. Read-only viewers (heatmap) zostają bez. Realny bug — § 86 ShotDrawer migration shipped bez `editable` → tap-place dead na prodzie; naprawione 2026-05-26 (`c61905e5`). Test: po podpięciu BaseCanvas + touchHandlerState, ZAWSZE sprawdź czy tap-action faktycznie wywołuje consumer callback (console.log w callbacku) przed merge.
- ❌ **NIE** emituj surowego `onSnapshot` snapshotu do listy UI bez ochrony przed **przejściowym pustym snapshotem z cache**. Z `persistentLocalCache` (`firebase.js`) listener może dostarczyć PUSTY snapshot prosto z IndexedDB (`s.metadata.fromCache === true`) zanim ciepły-cache/serwer go zrepopuluje — zimny/wyeksmitowany cache, koordynacja multi-tab na iOS Safari, albo chwilowy spadek łącza. Pusta tablica zeruje już-pokazane dane i **miga** w kółko: nazwy drużyn w match cards lecą na `?` (resolver short-circuit na pustym `scouted`), a efekt heatmapy ScoutedTeam (keyed na `teamMatches.length`) czyści wszystkie statystyki gdy `matches` na moment pustnieje; recovery dopiero na remount (tab switch). Wzorzec: `subscribeListSafe` (`dataService.js`) — pomiń pusty snapshot który jest `fromCache` GDY już dostarczyliśmy dane (pierwsza emisja zawsze przechodzi → loading się rozwiązuje; serwer-potwierdzone puste `fromCache:false` dalej czyści listę) + `onError`→`captureException` (wcześniej błędy listenera były połykane). Realny bug — P1 triage 2026-06-02 (`4f4c7765`, B25); `subscribeScoutedTeams`/`subscribeMatches` migrowane. Każda nowa list-subscription przez `onSnapshot` MUSI iść przez ten helper (lub równoważną ochronę). Bliźniaczo: `useGatedCatalog` (`useFirestore.js`) nie serwuje pustego cache jako fresh i nie cache'uje pustego fetcha (pusty wynik nie może zatruć 30d cache).
- ❌ **NIE** dodawaj nowego callbacka do `stateRef.current` bez weryfikacji destructure w **kilku** handlerach `touchHandler.js`. `handleDown:152` / `handleMove:367` / `handleUp:506` mają RĘCZNE destructure listy z `stateRef.current` — każda lista jest osobna i łatwo zgubić dodanie nowej propa w jednej z nich. Realny bug — `onDeleteShot` był w stateRef + wywoływany w `handleUp:653`, ale NIE w handleUp destructure → undefined → silent no-op (dead path, nikt nie testował bo mode='shoot' na main canvas był zawsze pod modalem przed § 86); naprawione 2026-05-26 (`c61905e5`). Process: przy dodaniu nowego call-out z stateRef.current → grep destructures w touchHandler i upewnij się że nazwa jest w **każdym** miejscu gdzie się jej używa.
