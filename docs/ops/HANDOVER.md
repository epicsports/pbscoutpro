...
**Last updated:** 2026-04-24 by CC implementation (§ 50 — Settings menu reorg + nav cleanup + Członkowie full UX, Tier 2 / 3 checkpoints, shipped overnight)
**Live app:** https://epicsports.github.io/pbscoutpro
**Repo:** https://github.com/epicsports/pbscoutpro
**Main HEAD at last update:** `0fe8739`

---

## 🚧 Currently in flight

**Nothing active.** Four ships across 2026-04-23 → 2026-04-24 (PPT + AUTH_ROLES_UNIFIED + ProfilePage § 33.3 + Settings reorg § 50); no stacked WIP branches. § 50 was a Tier 2 brief that consolidated the More tab into Jacek's exact six-section spec, retired the legacy BottomNav (Home/Layouts/Teams/Players object-based tabs), added the Wyjdź workspace self-leave path, and built the missing admin-side "Członkowie full UX" piece (UserDetailPage with link override + soft-delete via `users/{uid}.disabled` flag) — closing the CP4 gap from AUTH_ROLES_UNIFIED that had been resolved as "Path A verified" without admin override capability.
