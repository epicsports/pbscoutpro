...
**Last updated:** 2026-05-15 by CC (multi-device point-overwrite hotfix deployed at NXL Czechy day 1)
**Live app:** https://epicsports.github.io/pbscoutpro
**Repo:** https://github.com/epicsports/pbscoutpro
**Main HEAD at last update:** `3b236cf` (merge `fix/multi-device-overwrite`) — see `DEPLOY_LOG.md` 2026-05-15

---

## 🚧 Currently in flight

**Awaiting two-device smoke confirmation from Jacek** — multi-device overwrite hotfix deployed mid-tournament, needs floor validation (iPhone + laptop alternating saves on the same live match, indices 1/2/3 across devices, no overwrites). Code is structurally fixed and built/deployed clean; this is just the empirical green-light from real hardware. If smoke fails, the regression vector is `points.filter(p => p.coachUid === uid)` — verify `coachUid` is set on the new docs and `usePoints` is surfacing them to both devices.
