...
**Last updated:** 2026-05-19 by CC (Phase 1.3 migration script committed `e560151` ⏸️ awaiting Jacek run)
...
|---|---|---|
| 2026-05-19 | `e560151` (script only) | **Phase 1.3: Migration script — users.workspaces field deletion (§ 63.3 Option α).** ⏸️ Script committed, Jacek to run. New file `scripts/migration/phase_1_3_delete_users_workspaces.cjs` — Firebase Admin SDK, iterates `/users/{uid}`, deletes `workspaces` field via `FieldValue.delete()` from legacy docs. Idempotent. Init pattern matches existing `scripts/purge-anonymous-users.cjs` (`.cjs` + `GOOGLE_APPLICATION_CREDENTIALS` env var). Per Jacek waiver — backup procedure skipped, re-import as recovery. CC sandbox lacks service account JSON → run deferred per brief escalation gate #1. Once Jacek runs `--dry-run` then `--write` and confirms: flip Phase 1.3 to ✅ DONE + mark Phase 1 COMPLETE + promote Phase 2.1. Run instructions in DEPLOY_LOG 2026-05-19. |
