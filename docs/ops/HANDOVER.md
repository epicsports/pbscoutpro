...
- **CoachTabContent live-score parity** — `MatchCard` now accepts `liveScore` prop driven by Scout tab's `useLiveMatchScores` hook. CoachTabContent hasn't been updated to pass it; same `m.scoreA = 0 during LIVE` symptom would surface there. Cheap symmetry fix if Coach tab card scores look wrong on Saturday.
