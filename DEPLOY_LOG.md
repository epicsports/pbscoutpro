...
- **Harness:** emulator-only `/test/capture` rig (`TestCaptureHarness`, route gated on `VITE_USE_EMULATOR`, **dead in prod**) mounts the hook in tactic config + publishes `window.__pbCaptureTactic`. `capture-parity.spec` gains a TACTIC battery (single team, all 5 positional phases, place/runner/shoot[band+callout+precise]/bump, NO hit/elim) → 16-step byte-stable golden.
- **GATE proof:** Stage-1 POINT golden stays **byte-identical** (engine generalization is behavior-preserving for scouting); tactic golden reproduces; **FULL e2e 109/109**; build + precommit green.
- **Next (gated):** Stage 2.1 tactic doc shape + serialize + legacy `phases.breakout` compat (Q1/Q2). → Jacek GATE.

## 2026-06-20 — [REFACTOR/Tier-2] Tactic-as-Point Stage 1: extract useCaptureDraft (behavior-identical) (Opus brief, chat GO)
**App (auto-deploy, e2e-gated). No rules/data.** Merge `feat/use-capture-draft`. Also lands the approved design docs + Stage 0 discovery (`docs/architecture/TACTIC_AS_SCOUTED_POINT{,_STAGE0}.md`).
- **The shared capture brain** — MatchPage's ~280-line capture orchestration (draft state atoms, the stage-aware `draft`/`setDraft` routing seam, factories `emptyTeam`/`seedStageDraft`/`switchStage`/`stageDone`, every capture handler place/move/remove/shot/toolbar/zone/precise/select/elim/reason + the draw stack + undo, `toolbarItems`, `mirroredOpp`, the annotation seam) **MOVED VERBATIM** into `src/hooks/useCaptureDraft.js`. MatchPage destructures the return into the SAME names → `savePoint`/`SaveSheet`/`editPoint`/`resetDraft`/render consume it UNCHANGED. The cut line (§0): the hook owns the draft brain; savePoint/SaveSheet/outcome/side-stamp stay in MatchPage verbatim (the point does NOT consume an `eng.serialize`).
- **Point-preserving defaults** (`target='point'`, `teams='AB'`, `outcomeEnabled=true`). The one thin branch added now: `outcomeEnabled` gates the result-side toolbar items (default true → point identical). `'single'`/positional-phase params + serialize/hydrate exports DEFERRED to Stage 2 (where the tactic editor exercises them — adding untested branches to the NXL-proven path now would itself risk parity).
- **PARITY (the non-negotiable, proven):** a golden-master harness (`tests/e2e/capture-parity.spec.js` + emulator-only `window.__pbCapture` probe, dead-eliminated in prod) drove the REAL capture path over the §4 battery (place/runner/shoot/zone/bump/hit/reason × break/settle/mid/endgame × both teams × outcome) and recorded the deterministic draft tree BEFORE the move; after the move it reproduces it **byte-identical**. **FULL emulator e2e 108/108** (capture/save/edit/review/concurrent-merge/stats/phase-view). build + precommit green. MatchPage functionally byte-identical.
- **Next (gated):** Stage 2 = tactic editor on the shared engine (`target='tactic'`, outcome=off, single). → Jacek GATE.
...
- **FIX 2 — remove ✕ corner collision.** The active-row "remove from board" ✕ shared the rail's top-right corner with the §116 collapse/close control → mis-tap. **Fix:** replaced with the app's existing `SwipeDelete` (swipe-left), parameterized (additive `label`/`bg`/`color`, all existing call sites unchanged) to a NON-destructive amber **"Remove"** (stays in the library) — §27 keeps red for destructive only. Drag handle `stopPropagation`s `touchstart` so a vertical reorder drag never triggers the horizontal swipe.
- **FIX 3 — edit door.** Added a Move (positions) fieldTool next to the present pencil → navigates to the structured `TacticPage` editor (`/layout/:id/tactic/:id`); the pencil stays present/annotate.
- Gate: precommit + `tactics-board.spec.js` **3/3** (new test enters present → asserts a sized field, not black; edit door opens the editor). §27 PASS.
- **Smoke owed (Jacek, prod):** open a tactic → pencil = full-bleed present renders the FIELD (not black) + draw saves; Move icon = opens the editor; swipe a row left → "Remove" → tactic leaves the board (library re-add still works); no mis-tap with the rail close.

## 2026-06-20 — [FEATURE/Tier-2] Coach Tactics board (rail-native) + one contextual entry (Opus brief, chat GO)
**App (auto-deploy, e2e-gated). NO rules change** (the `/layoutOverlays/{lid}/tactics` catch-all is already `isCoach`-write with no field allow-list, so the new `onBoard`/`order` keys write as-is). **NO data migration** (legacy-safe client-side read). Merge `feat/coach-tactics-board`.
- **New rail-native board** (`LayoutTacticsBoardPage`, `/layout/:layoutId/tactics`) — completes the `tactic` archetype shell. BROWSE = `CanvasRailLayout`: field HERO = read-only `InteractiveCanvas` preview of the ONE selected tactic; rail = the ordered on-board list (tap=select, drag-handle=reorder, ✕=remove). PRESENT/annotate = **full-bleed §76 immersive** draw (chrome hidden, floating `DrawToolbar`) — the "show to players + annotate" surface; persists via `updateLayoutTactic`. "+ New tactic" footer (blank→present) + library picker (re-add off-board tactics).
- **Data model (additive, legacy-safe):** tactic docs gain `onBoard` (bool) + `order` (number). `addLayoutTactic` defaults `onBoard:true`, `order=max+1`; new `reorderLayoutTactics` writeBatch. Board read is **client-side** (`utils/tacticState`: `onBoardTactics`+`sortBoardTactics`), NOT Firestore `orderBy` — legacy docs lack the fields → missing `onBoard`=TRUE, missing `order`→createdAt fallback. **Existing tactics appear by default.** Remove = `onBoard:false` (stays in the library, NOT deleted). `normalizeFreehandStrokes` lifted to `utils/tacticState` (TacticPage reuses it).
- **One contextual `<OpenTacticsAction>`** (`✎ Tactics board`) in tournament view (CoachTabContent), layout/Konfig (LayoutDetailPage), training (TrainingCoachTab), coach home (MoreTabContent→Playbooks). Resolves context→layout (explicit id → go; none → 1:go / 0:/layouts / N:picker). Tactics stay **layout-sourced** (no per-event duplication). LayoutDetailPage's tactic list **COEXISTS** (structured position-editing door → `TacticPage`).
- **Two scope calls:** "full-bleed present" = §76 immersive (NOT §116 *manual* collapse — F2 unbuilt; same end-user result); in-board present = freehand annotation (structured positions stay in TacticPage). DESIGN_DECISIONS §124, FIELD_VIEW_ARCHETYPES, fieldViewConfig `tactic-board`.
- Gate: precommit + `tactics-board.spec.js` (data-contract + UI-mount) **green** on the emulator (the UI test caught + fixed a not-found-guard bug keyed off the wrong loading flag). §27 PASS.
- **Smoke owed (Jacek, prod):** open the board (Konfig / tournament / training / More) → select a tactic → preview renders → reorder persists → ✕→library→re-add → "+ New" → full-bleed draw saves.
...
- **Members-screen crash `undefined is not an object (evaluating 'd._result.default')` (bug 2):** the React.lazy payload signature of a **stale page chunk** 404ing after a deploy (Jacek's iOS PWA held an old `index.html` across tonight's many deploys). The app already self-heals stale chunks, but `STALE_CHUNK_RE` only matched the network-style import messages ("Failed to fetch dynamically imported module" etc.), so this surfaced as a white screen instead of a loop-guarded reload. **Fix:** added the `_result.default` signature to `STALE_CHUNK_RE` + test the error **stack** (not just message, for Chrome's generic phrasing). The `ErrorBoundary onError` now triggers the one-shot reload.
- **Misplaced/unfillable forms (bug 3) — NO code cause found.** The shared `Modal` (bottom-sheet, `position:fixed inset:0`, visualViewport keyboard-lift) is unchanged, and no Modal ancestor has a `transform`/`filter`/`contain` containing-block trigger. Strongly suspected to be the **same stale-bundle/version-skew** as bug 2. Triage: a fresh bundle should clear it; if it persists on a clean load it's a real iOS-PWA keyboard/modal-lift issue → follow-up.
- **⚠ Self-heal caveat:** the fix is itself a chunk change, so it can't rescue the ALREADY-stale session — **one hard reload / PWA reopen** is needed to pick up the new bundle; thereafter the crash auto-reloads.
- Gate: precommit + **FULL emulator e2e 98/98**. §27 PASS.
- **Smoke owed (Jacek, after a hard reload):** (1) drawer shows ONE workspace switcher; (2) members screen opens (no white screen); (3) add team/player form sits correctly + is fillable.
...
- **Follow-up `b6ce1ae1`:** the button was mounted in `MainPage`'s `AppShell` → only on the `/` route. Moved the single mount to `App.jsx` (alongside `OfflineBanner`/`ViewAsIndicator`, inside `HashRouter` after `Routes`) so it floats over EVERY authenticated screen; removed the `MainPage` mount (no double button). Still `useIsSuperAdmin`-gated.
- Gate: `npm run precommit` (build + lint-ui) passed for all changes; e2e runs in the deploy workflow.
- **Smoke owed (Jacek/Majma, prod):** Majma reloads → lands in an open tournament with events; super-admin sees the ⌖ button on every screen and a snapshot exports.
...
- **Two consumer bugs fixed (one root cause).** `capturePhases()` returns phase OBJECTS; `.map(toPersistedLiteral)` passed objects where a key string was expected → `undefined`. (a) MatchPage `PHASE_SEGMENTS` → `PHASE_ICON[obj]` undefined → `<SegIcon>` undefined → **"Element type is invalid … render method of MatchPage"** ErrorBoundary crash (the phase-view regression / the Sentry class). (b) dataService `STAGE_ORDER` → object stage keys never matched → merged `timeline.length 0` (concurrent-merge). Both → `.map(p => toPersistedLiteral(p.key))`. NOTE: prod `main` never had `capturePhases` (literal `['break','settle','mid']` + complete icon map), so the live Sentry report was a stale-cache/pre-fix artifact, not current-prod code; this merge closes the class going forward.
- **Scope per ratified D4 (2026-06-16):** capture is the SUBSET break/settle/mid. `endgame` ships `captureEnabled:false` (reserved like `preBreakout`/`outcome`) → in the model axis, in NO capture/report UI; read-side endgame branches are inert, data-gated forward-compat. The older inline brief's "build Endgame capture" was superseded by the ratification (doc = source of truth).
- Doc: `POINT_AS_TIMELINE.md` §2 D4 notes `pointPhases.js` as the single source. Gate: precommit + **88/88 functional e2e** + phase-view/concurrent-merge/matchreview-rail/scoutedteam-rail. §27 PASS (refactor; no new visual surface).
- **Smoke owed:** open a scouted/match point with Settle/Mid keyframes in review → phase row Break/Settle/Mid switches + ▶ play, no crash; two-coach concurrent end-match merge reflects both sides.
...
- **`generateInsights`:** `computeBreakSurvival` + `computeShotTargets` take a `stage` param (default `'break'` = today's numbers → no regression); new `computeEliminationReasons`. Shared module-scope `kfOf` + `stageSideData` mirror the callout-zone per-stage idiom — Break = kf#0; Settle = settle keyframe (legacy point-level fallback so old points still populate); Mid = mid keyframe (no fallback → graceful empty). Break shooting stays merged kf#0 (parity); Settle = obstacle band; Mid = mid-keyframe band.
- **FINDING (premise correction):** the brief assumed "read-side only, the data is already there," but the normalizer `mapOnePointForTeam` **DROPPED** the per-keyframe `eliminationReasons` — so it was NOT at the read site. Added the one-line carry through normalization (still zero-writes, read-side). The captured-but-invisible reasons are now surfaced — the highest-value piece.
- **UI:** `ScoutedTeamPage` memos depend on `hmPhase`; new elim-reason block reuses the Breakouts/Shooting table-row idiom (neutral data styling, §27 — no amber). `SegmentedControl` gained an optional per-item `testId` (additive). i18n `section_elim_reasons` (pl+en); reason labels reuse the existing 2b `reason_*` keys.
- **e2e:** `pat-stage25-report.spec.js` — (1) bridge per-stage distinctness (breakouts/shots/reasons follow the stage + graceful-empty on a no-timeline point), (2) UI proof the global control toggles the reason block per phase (TRN_PHASE seeded settle=gunfight/mid=penalty). Functional gate **86/86**; §27 self-review PASS.
- **Smoke owed (Jacek):** open a scouted team with Settle/Mid points → switch Break/Settle/Mid → confirm Breakouts/Shooting numbers + the "why eliminated" block track the phase.
...
- **Coverage (--dry, stable vs 2026-06-16 discovery §8.1):** 24 orphan · 21 overlay-live · 9 tournament-live. Of the 24: **19 delete-able** (same-id twin already at `layoutOverlays`, from STAGE-3 copy) + **5 stranded** (no twin — real tactics created on the dead path post-STAGE-3; on 3 ranger1996 layouts `AH6dEG…`×2 / `OajoxCM…`×1 / `kwamYyv…`×2).
- **Sequence:** backup all 24 (full data → `…/dk/pbscoutpro_backups_tactics/<ts>/orphan_tactics_backup.json`, outside-repo/gitignored) → copy-by-id the 5 stranded → layoutOverlays (raw shape, legacy `steps` is a clean subset the overlay reader renders) → **verify all 5 twins exist** → delete all 24. **Final census:** `layouts/tactics` 0 · `layoutOverlays` **26** (21+5) · `tournaments` 9 untouched.
- **NOTE (flagged):** the 5 migrated tactics were previously invisible (orphan path has no reader); they now surface on those 3 ranger1996 layouts — this is intentional data-recovery (losing user tactics > surfacing them). Reversible via the backup if undesired.
- **OWED (OP1):** retire the tournament-tactics store per §97.5 — migrate the 9 + repoint/remove `subscribeTactics`/`addTactic` + update the TacticPage tournament route. Code+data Tier-2; needs the brief's reader-migration + layout-resolution rules (not improvised). `docs/architecture/TACTICS_TWO_STORE_DISCOVERY.md`.

## 2026-06-17 — [FEATURE] Reads Mini "Take a Break" game + global leaderboard (§117 STAGE 2)
**App (auto-deploy via deploy.yml) + Firestore RULES (manual, deployed).** Tier-2 (new feature, §117 LOCKED spec; GO'd "anything waiting on go" 2026-06-17 — STAGE 1 docs had explicitly awaited GO for STAGE 2 build). Merge `186071e6`.
- **Game:** Game&Watch-style catch game shipped into the app — 4 lanes, amber falling balls, slide-catcher, 3 misses = game over, speed ramps with score, Game A (3 lanes) / Game B (4, faster), misses reset at 200/500, flash each ×100, score 0–9999 (7-seg). **Pure DOM/CSS + SVG — no canvas, no game engine, no new dep**, lazy own chunk (`ReadsMiniPage` 17 kB / 5.9 gz). Arcade layer: attract loop (title ↔ HIGH SCORES) + 3-letter initials on a personal best. Audio: WebAudio SFX + bg music loop (graceful if asset absent), music/sfx mute toggles, `prefers-reduced-motion`. Art split into swappable layer components (GhostGrid/Conveyors/Feeders/Catcher/Ball/Splat) so hand-drawn art can replace a layer later.
...
- **RULES (deployed):** `match /leaderboards/{board}/scores/{uid}` — read = any authed; create/update own-doc only, int 0..9999, `[A-Z]{3}` initials, monotonic score, 5s write cooldown, no delete. `firebase deploy --only firestore:rules`, compiled clean, released. §117.6 deliberate exceptions: global board crosses workspace isolation BY DESIGN (recreational, no scouting data) + arcade initials (no PII/GDPR). **NEW top-level collection — NOT a tenant-isolation predicate change.**
...
- **e2e:** `reads-mini.spec.js` — full UI flow (drawer entry → attract → start → playing controls → GAME OVER → back) + leaderboard WRITE against the REAL rules via the emulator bridge (create/update, reject-lower, **cross-uid write DENIED**). Functional gate `npm run test:e2e` **84/84**. Test bridge dead-eliminated from prod (dist grep for `__pbtest` clean).
- **OWED:** (1) **audio asset** `public/sounds/sky-catcher-loop60.m4a` — drop it in (60s AAC loop); until then bg music degrades silently (SFX work, synthesized). (2) **STAGE 3 App Check** (reCAPTCHA v3 web, app-wide enforcement) — separate GO.
...
- **e2e:** `layout-tactic-freehand.spec.js` — freehand round-trips through `addLayoutTactic`. Fail-first proven RED (`hasFreehand=false`) → GREEN. Suite 75/0.
- **Discovery (read-only, brief-ready):** two-store consolidation mapped. **Prod audit (admin-SDK read):** tournament-store = **9 real tactics** (ranger1996, 5 tournaments) → full consolidation (retire the tournament store per § 97.5) is **Tier-2** (needs `--live` migrate+backfill + Opus brief + Jacek GO), NOT a free path-retire. Registered in NEXT_TASKS.
- **i18n triage (read-only):** the lint's "Polish string detected" warnings are ~94% false-positive (884 are the i18n.js dictionary's own PL values + 189 are t()-fallback strings); genuine hardcoded-Polish UI debt is only ~68 strings (CSVImport/ScheduleCSVImport/PlayerStats lead) — a modest future i18n batch + a lint-refinement opportunity (skip i18n.js + t() lines in the Polish rule). The EN-literal guard (rule 2b) is clean (5 hits, ~2 actionable aria-labels). Noted for the morning, not shipped.
...
- **Diagnosis (admin-SDK read, `scripts/diag/invite-format-audit.cjs`):** biuro `emailVerified=true` (rules `email_verified` gate NOT the blocker); invite email-keyed + pending + correct; invites collection has **no format split** (2 email-keyed + 16 token-keyed in separate id-spaces — `getDoc(invites/{email})` resolves cleanly; `createEmailInvite` always email-keyed). **Real blocker:** biuro is **already in `ranger1996.members`** (empty roles, from the old pending-approval flow), and the self-claim rule required `!(uid in members)` → the workspace write was DENIED → permanent pending. Jacek's `!isEmailSignInLink()`-gate hypothesis ruled out (the claim runs on the post-set-password `#/` load; e2e proves a fresh invitee is claimed).
- **Fix (rule, CONFIRM):** self-claim membership clause `!(uid in members)` → **`resource.data.userRoles.get(uid, []) == []`** — a fresh non-member OR an already-member with empty roles may claim; can't overwrite a non-empty role (the `userRoles[uid] == [invite.role]` + own-key-diff guards still bound the result; verified-email + own-email pending-invite-for-this-slug unchanged).
- **Fix (code):** `EmailLinkSetupPage` now claims the invite **explicitly at activation** (after set-password, before handoff) — no longer relies on the post-reload background effect; background self-claim stays as the gated retry/no-link fallback.
- **e2e:** `already-member-claim.spec.js` (member+empty-roles+pending-invite → login → self-claims role → in). **Fail-first proven** RED (old rule) → GREEN. Full suite 71/0.
- **biuro self-heals on next login** (background self-claim now permitted by the rule) — no manual grant needed. **Owed: Jacek smoke** — biuro relogin → lands in ranger1996 as coach; + the email-invite smokes from the prior entry still stand (Maks/Tymek cold-open; one fresh email invite end-to-end).

## 2026-06-15 — [FEATURE] Durable email-link invite onboarding (ships the onboarding arc)
...
- **Flow (Spark-native, no backend):** admin enters invitee email + role → `invites/{email}` pending + `sendSignInLinkToEmail` (Firebase emails the magic link) → invitee clicks in ANY browser → `EmailLinkSetupPage` express-reg (complete email-link sign-in + set password) → on `#/` reload the email-keyed self-claim writes `workspaces/{slug}.members + userRoles[uid]=[role]` (the authoritative store) → lands in with role. Browser-agnostic (keyed on the authed email, not a token). Admin "Zaproszenia e-mail" view on MembersPage + token open-link kept as fallback.
- **#166 root cause + fix (the CI-red the branch carried):** `signInWithEmailLink` authenticates mid-express-reg (before set-password), prematurely firing the WorkspaceProvider background effects — auto-enter (threw on the rules-denied legacy first-entry write) + the self-claim (`reload()`), which re-loaded the still-active email-link URL → looped the invitee back to the empty confirm step. **Fix:** gate both effects on `!isEmailSignInLink()` (`useWorkspace` + `useInviteRedemption`) — the express-reg page owns the session until it replaces the URL with `#/`.
- **Rebased onto source-of-truth (`4ddbf0b2`):** self-claim writes `workspace.userRoles` (authoritative for gate + entry, per today's earlier fix) — an invited user rides the same fixed entry logic, never the pre-fix shape.
- **Rules (deployed, Jacek CONFIRM — TENANT-ISOLATION):** two additive branches. `/invites/{email}` claim + `/workspaces/{slug}` self-claim, both gated on `request.auth.token.email_verified == true` (the email-link flow sets it) + own-email match + own-uid + exact invited role + matching pending invite for this slug. Can't escalate, can't touch other members. No existing rule tightened.
- **e2e:** `email-link-invite` (cross-context express-reg), `email-invite-claim` (no-token self-claim), `bulk-invite-claim` (NEW — two verified invitees self-claim a shared ws in parallel, multi-account window), `admin-pending-invites`. Functional suite green (one unrelated `hitability-responsive` flake passed isolated; CI retries=2 absorbed it).
- **OWED — Jacek (STEP 5+6, deliberately manual):** (a) grant **`biuro@epicsports.pl`** a role FROM THE APP (Members → assign) — proves the in-app grant→entry path post source-of-truth fix; if it doesn't land them in, that's a real bug → report. (b) **Smoke:** Maks + Tymek cold-open → in ranger1996; one fresh email invite end-to-end (enter email → recipient gets Firebase email → activates → lands in, in a different browser than clicked).
...
## 2026-06-15 — [FIT-blocker fix] userRoles authoritative for workspace ENTRY (ranger source-of-truth strand)
**HEAD `4ddbf0b2`** (merge; feature `fix/role-source-truth-entry`). **App-only** (+ e2e + diag tooling). Tier 1 (Jacek GO; logic-only, **NO rules change**). Functional e2e **66/0** (new spec fail-first proven RED→GREEN); build + precommit green. Revert: `git revert 4ddbf0b2`.
...
- **STEP-1 verdict (corrects the brief's hypothesis):** the gate ALREADY reads `workspace.userRoles[uid]` (both `roles` and `isPendingApproval` via roleUtils), and `linkSkippedAt` ALREADY decouples profile-linking from access (App.jsx:137). The split was **not** in the role store — it was in **ENTRY**: with `defaultWorkspace` null, `autoEnter` fell back to the `members array-contains` query and entered **`docs[0]` (doc-ID order)**, which for a >1-membership user can be an empty-roles ws → PendingApproval, even though another ws held the roles. (Also: the `users/{uid}` self-update rule allows only `displayName/email/linkSkippedAt`, so `roles`/`defaultWorkspace` can't be client-synced on grant — the brief's option (b) is rules-blocked; option (a) chosen.)
- **Fix:** `autoEnterDefaultWorkspace` now **prefers a membership where `userRoles[uid]` is non-empty** (falls back to `docs[0]` only if none carry roles) → `userRoles` authoritative for entry as well as the gate. No cross-user write, no rules change.
...
- **Recovery (STEP-3, --live, Jacek GO):** stamped `users/{maks-uid}.defaultWorkspace=ranger1996` + mirrored `roles` (admin-SDK; client can't write these per rules). Robust across ALL bundle versions (every build since 2026-04-24 enters via the `defaultWorkspace` pointer) → lands him in ranger1996 next open regardless of cache. `scripts/diag/recover-stuck-user.cjs` (dry by default).
...
- **OWED:** (a) **Jacek GO to batch-stamp the 3** (or just tymek). (b) `biuro@epicsports.pl` needs a role GRANT (not a stamp). (c) **Maks prod smoke** — relogin / cold-open PWA. (d) **email-invite branch (`feat/email-invite-association`) stays HELD** — rebase onto this main, then green `email-link-invite` (remaining failure = a distinct auto-enter throw in the express-reg path).
- **Deferred (Jacek):** adding `defaultWorkspace` to the `/users/{uid}` self-update rule allow-list (would persist the auto-enter stamp) — not load-bearing given the membership fallback; rules-change, deferred.

## 2026-06-14 — [FIT-blocker fix] Invite token in localStorage + invite→register repro (Maks #3)
...
- **Diagnosis (verified by repro):** the manual "Utwórz konto" step does NOT break it — `tests/e2e/invite-register-associate.spec.js` reproduces the exact `#/invite/{token}` → switch-to-register → register flow and is GREEN (account associated, lands on onboarding, not NoWorkspace). Root cause is **token loss across contexts**: the invite token was stashed in **sessionStorage** (per-tab) and lost when the link was opened in one tab/app and the account finished in another (or reopened later) — the likely shape for players opening links from chat/email apps.
- **Fix #2 (this commit):** stash the invite token in **localStorage** (`useInviteRedemption.js`) — survives tabs + sessions in the same browser. Honest limit: cross-BROWSER / chat-app in-app-browser hops still can't carry a client token (covered by the durable follow-up below).
- **NOT done (blocked / deferred):** writing `users/{uid}.defaultWorkspace` on redeem — the users self-update rule allows only `displayName/email/linkSkippedAt`, so it'd be permission-denied (and isn't load-bearing: `members[]` + the auto-enter members-query already associate). Would need a rules CONFIRM; skipped.
- **STILL OWED (Czechy-critical):** (a) **recovery of the existing stuck accounts** — admin-SDK associate (Jacek to supply `{UID→workspace→role}` + GO; --live write). (b) **Durable #3** — record invite-intent server-side + a super-admin "pending invites / unassociated accounts" view so a lost client token is recoverable and admin can always grant the first role. Structural — spec for Jacek, not built autonomously.

## 2026-06-14 — [FIT-blocker fix] Self-claim picker shows the FULL player catalog (Maks #2)
**HEAD `311a2eb0`** (merge; feature `fix/self-claim-full-catalog`). **App-only** (+ e2e). Tier 1 (Jacek-directed; logic-only, **NO rules change**). Functional e2e green (new spec, fail-first proven RED→GREEN). Revert: `git revert <merge>`.
- **Bug (Jacek):** after the permission-gate fix, Maks opened the player-matching search and saw **nobody** — should see the whole player DB. **Root cause:** both self-claim pickers (`PbleaguesOnboardingPage` onboarding gate + `ProfilePage` claim) restricted candidates to `players.filter(p => mySlugs.has(p.ownerWorkspaceId))` (§85 "defense-in-depth" UI filter) — empty when the user's workspace owns none of the candidate profiles, AND `if (mySlugs.size === 0) return []` (e.g. before `useUserWorkspaces` resolves). Maks (Ranger player) saw none because the filter, not the rules, hid them.
- **Fix:** both pickers now pass the FULL global catalog (`usePlayers()`), dropping the workspace filter + `useUserWorkspaces`. Rules-safe — `/players` read is open (`auth != null`). `matchPlayers()` shows the unlinked roster on empty query + searches all on a query.
- **Rules note (NOT changed):** the §85 self-link WRITE carve-out still gates claims to `isMember(resource.data.ownerWorkspaceId)` — own-workspace profiles claim fine; claiming a profile owned by ANOTHER workspace would be denied. If cross-workspace self-claim is needed, that's a separate §85 tenant-rules change (**Jacek CONFIRM**). **Owed: Maks smoke** — confirm he now sees + can claim his (Ranger-owned) profile; if his profile is owned by a different workspace, ping me for the §85 CONFIRM.
- e2e `player-self-claim-catalog.spec.js` (B4_PLAYER — unlinked player in a workspace owning zero players) proves the picker surfaces the global roster.

## 2026-06-14 — [FIT-blocker fix] Maks permission-gate: workspace-listener robustness + recovery gate
**HEAD `96504036`** (merge; feature `fix/maks-permission-gate`). **App-only** (+ e2e seed/bridge). Tier 1 (regression fix, logic-only — **NO rules change**, so no CONFIRM gate). Functional e2e green (incl. new contract spec; one unrelated quicklog flake passed on isolated re-run); migration-diff 24; precommit green. Revert: `git revert <merge>`.
- **VERDICT (root cause):** NOT the rules. The workspace read allows any member (`uid in resource.data.members`, firestore.rules:247) and `users/{uid}` read is open — a pending member CAN read (proven: the new contract e2e passes on a clean pending member). The defect is **listener robustness**: `useWorkspace`'s workspace `onSnapshot` had **NO error callback** (and the `users/{uid}` one an **empty** one), and neither re-attached (effect deps = `[slug]`). A transient permission-denied at attach — e.g. the listener attaching a beat before the self-join membership write lands — **killed the listener permanently**; a later admin role-grant then never propagated → stuck on "poczekaj aż admin" forever. This is Maks.
- **Fix (STEP 3):** both listeners get real error handling + the workspace listener gets **bounded auto-re-attach** (3× backoff) — self-heals the transient attach-race without any user action; persistent failure sets `permissionError`.
- **Fix (STEP 4, unconditional):** `permissionError` (set on persistent listener error, or a 12s no-snapshot backstop) renders a new **`PermissionErrorScreen`** ("Nie udało się sprawdzić uprawnień — odśwież") with retry+signout — the no-eternal-loading principle applied to the permission gate; a swallowed listener error can never strand a user again.
- **Coverage (STEP 5):** `tests/e2e/pending-approval-refresh.spec.js` proves the live-refresh CONTRACT (pending member + admin grant → gate clears → app, no reload). The recovery-path e2e was dropped: the Firestore **emulator does not deliver permission-denied to an active listener on mid-session revocation** (verified — removed member stays frozen, no error event) and getDoc shares the listener channel, so the error path can't be induced in-emulator. PROD errors on attach-time denial (Maks's case) → re-attach + recovery fire there. Recovery is build/logic-verified.
- **Owed: Jacek + Maks prod smoke** — have Maks relogin in his PWA: the re-attach/recovery should land him in the app without us touching his device. Also a general pending-invite→grant smoke before NXL Czechy.

## 2026-06-14 — [arc-B migration] UserDetailPage → <Screen archetype=detail> (640)
...
- **Unblocked the deferred page:** the prior attempt hit 68px content noise (async `getDoc(profile)` raced the 1500ms screenshot wait). Fixed deterministically: (1) `data-testid="user-detail-loaded"` on the loaded content + a per-page `waitFor` in the harness (captures the loaded state, never the spinner); (2) the super fixture's `createdAt` pinned to a FIXED epoch (was live `now`) — UserDetail renders "Dołączył {createdAt}" and the gate reseeds per run, so a live timestamp flaked that row. Inner content keeps its 720 cap below desktop (phone+tablet identical); only desktop narrows to 640.
- Preserved `background: COLORS.bg` + `minHeight: 100dvh` (not Screen's default 100vh) via the `style` prop; `padBottom` default (old shell had `paddingBottom:80`). **Owed: Jacek smoke** — a user-detail page on desktop (640) vs phone/tablet.

## 2026-06-14 — [HOTFIX] i18n t('k')(args) auto-invoke crash class + lint + pagination e2e
...
- **Prod crash (Jacek):** AdminPlayersPage "Strona undefined z undefined" / `t('admin_teams_page_n_of_m') is not a function`. **Root cause:** `useLanguage`'s `t()` AUTO-INVOKES function values (`t(key, ...args) => typeof v==='function' ? v(...args) : v`), so the footgun `t('k')(a,b)` invokes with NO args first (→ a string), then calls that string → crash. Correct shape: `t('k', a, b)`. **Genuine source bug** (my first-pass "stale cache" read was WRONG — node bypassed t(); the e2e caught what static analysis alone couldn't).
- **10 footgun call sites fixed** (`t('k')(…)` → `t('k', …)`): AdminPlayers + AdminTeams pagers, all 4 MergePlayersModal interpolators (**would crash the merge-players flow on open** — never e2e-covered), `ppt_step_counter` (also a MISSING key, added), `reset_sent`, `line_draw_banner`, `user_global_role_promote/revoke_body` (last 4 were ALSO `{placeholder}` static strings → converted to function interpolators in pl+en).
- **Lint (durable gap closed):** `scripts/lint-i18n-shapes.js` wired into `npm run precommit` — Rule A flags `t('k')(…)` chained invocation (crash footgun); Rule B flags `t('k', …args)` where `k` isn't a function in both langs (silent arg-drop → literal `{placeholder}`). The t()-WITHOUT-SCOPE lint remains separately owed (sibling, different mechanism).
- **Coverage (twin of the Quick-log gap):** pagers only render at `totalPages>1`; the 15-player seed never paginated. Seed +40 padding players (55 → 2 pages, `teamId:null` so they hit only the two unfiltered global lists); `tests/e2e/admin-pagination.spec.js` exercises the pager — **fail-first organic: RED on the buggy code, GREEN after.** Re-baselined `players` + `admin-players` snapshots (the prior admin-players baseline had captured the crash screen).
- **Owed: Jacek prod smoke** — AdminPlayers/AdminTeams pagination (>1 page), the super-admin merge-players modal, LoginPage reset-sent toast, UserDetail promote/revoke, LayoutDetail line-draw banner.
...
- **i18n regression-sweep** — 14 leaked strings in MatchPage/ScoutedTeam/QuickLogView (the mixed PL/EN Jacek saw on "done" screens): OPEN/from/No point/Next point/Same/Swap sides/Penalties/Overtime, eliminated/Manage roster/No matches/Empty roster, WIN. HERO + PENALTIES values + outcome-domain strings left as DATA.
- **B7** — login + player-edit forms (LoginPage/PlayerEditModal/MergePlayersModal): mapAuthError(err,t) keyed; NATIONALITIES left as domain data; MERGEABLE_FIELDS gained labelKey. pwa-offline spec → login-offline testid (PL copy broke the English text match).
- **B9** — detail pages (TeamDetail/ScoutDetail/ScoutIssues/BunkerCard): ScoutIssues TYPES gained labelKey; "Snake Beam"/"HERO" = domain identical.
- All three: extraction/translation commit split + EXPECTED_DIFF_REGISTER rows. i18n dict 28 duplicate keys = baseline (no new dups introduced). **Owed: Jacek smoke** — login screen + player-edit/merge modals + team/scout detail pages po polsku; offline card on network drop. **Remaining i18n: wave 2 = B8 (imports)/B10 (analytics)/B11 (admin)/B12 (components), ~109 strings.**

## 2026-06-13 — [NIGHT RUN cont.] H1 batch 4 (match & scout) — H1 EXTRACTION BATCHES 2–6 COMPLETE
**HEAD `63c1296e`.** **App-only.** Tier 1, night blanket GO. Full emulator e2e **48/48 green** post-merge. ~50 keys (`match_*`/`scouted_*`/`quicklog_*`), extraction/translation split + register row. **MatchPage** (confirm modals, ActionSheet labels incl. "End match (mark as FINAL)", scoreboard Scout/Quick, quick-note, bottom-sheet) + **ScoutedTeamPage** (error/loading/empty states, layer-rail pills, HIGH badge) + **QuickLogView** (menu labels). `ui.jsx` ActionSheet items gained optional `testId`. **Specs moved to testids** to survive translation: `phase-view` (end-match-final-action), `scouted-team-loader`, `team-detail-loader` (+ match-load-error/scouted-load-error wrappers); **one follow-up fix in the merge:** the Retry-button assertion was still `/Retry/i` but `match_retry` PL = "Ponów" → made language-agnostic (`/Retry|Ponów/i`). **i18n coverage now ~all major surfaces; 28 pre-existing duplicate keys remain (hygiene ticket).** **Owed: Jacek smoke** — MatchPage/ScoutedTeam/QuickLog po polsku; potwierdzić że phase-view (segmenty/podgląd/replay) działa po przetłumaczeniu.
