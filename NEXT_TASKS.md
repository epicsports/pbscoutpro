...
**Status:** 🎉 Phase 1 schema foundation COMPLETE. 🎉 Phase 2 Step 1 (Leagues) COMPLETE 2026-05-19 (`324f380` + `2f81b2b` + `96e9951`). 🚨 Pending: `firebase deploy --only firestore:rules` for /leagues/ rules. ✅ **Phase 2.2.a EXECUTED 2026-05-19** (`ab1319c` + execute run) — `/players/` global collection populated with 934 docs (Option α preserve IDs, 42 dedup pairs collapsed). **Next: Phase 2.2.b (workspace consumption refactor — usePlayers hook with global reads + alias resolution) — brief writing pending.**
...
- **🎯 Phase 2.2.b — Workspace consumption refactor (usePlayers hook + alias resolution)** — App code change. Build `usePlayers` hook reading from global `/players/` (no constants fallback — no constants exist for players). Workspace consumers (PlayerEditModal, KIOSK, TeamDetailPage, MatchPage point.assignments resolution, etc.) refactored to use hook. Alias resolution layer: 42 canonical-vs-alias mappings need lookup so any `point.assignments[]` reference to an alias ID resolves to canonical global player. **🚨 Pre-req:** Firestore rules for `/players/{playerId}` need `allow read: if request.auth != null` (currently default-deny — hook would fail without it). Reference: § 63.15.3. **READY FOR CC BRIEF WRITING — next strategic action.**
- **Phase 2.2.c — Super admin UI for global players CRUD** — UX work, follows 2.2.b stable. CRUD on `/players/` gated to super admin per § 63.15.3 access control. Pattern reuses Phase 2.1c shape.
- **Phase 2.2.d — Legacy `/workspaces/{slug}/players/` cleanup** — DEFERRED until 2.2.b consumption migration stable. Migration script to delete subcollection.
