...
**Last synced:** 2026-05-25 · main HEAD `86f98a85` (post § 84 B2-hotfix b+a — SHIPPED + deployed)
...
| **B2 (c)** | high | First-login (new account) | **STILL OPEN — architectural decision needed.** `selfLinkPlayer` writes workspace-scoped `/workspaces/{slug}/players/{pid}`, `subscribeLinkedPlayer` listens workspace-scoped, but the picker (via global `usePlayers`) shows global `/players/{pid}` ids that don't necessarily exist in the workspace subcollection. For workspaces ≠ `ranger1996`, linking still fails — § 84 hotfix only prevents the user from being permanently stuck. | Pick the canonical contract: workspace-scoped (modal feeds from workspace player listing) OR global (`selfLinkPlayer` + listener migrate to global + `firestore.rules` self-link carve-out added on global). Touches dataService + rules + onboarding + ProfilePage. **Needs Opus design pass.** | awaiting-decision | Jacek 2026-05-12 (NEW ACCOUNT #1) + High-3 diagnosis ESCALATE |
...
| **B11** | med | ShotDrawer (Match scouting) | Three symptoms = one § 75 grammar drift: (1) dead X icon next to placed precision shots, (2) pan in precision mode broken — drag places another shot instead of panning, (3) deletion only via UNDO (no tap-element→menu, no drag-to-move). | `ShotDrawer.jsx` is not a canvas — `<img>` + native scroll + absolute `<div>` markers with `pointerEvents:'none'`. Dead X lives in `drawPlayers.js:124-129`. Per 2026-05-24 decision: MIGRATE to BaseCanvas + specialized wrapper with `viewportSide='opponent-half'` clipping, NOT local patches. | open (active queue #2) | NEXT_TASKS A2 |
...
- **§ 84 B2-hotfix onboarding funnel hang** `86f98a85` — async hygiene (`finally { setBusy(false) }` + 8s watchdog + removed busy-guard on skip) + modal-side error reflow + persistent Skip/Logout above modal backdrop (z:110). B2 (c) collection contract still open — awaiting Opus design pass.
