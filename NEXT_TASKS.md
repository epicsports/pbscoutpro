...
**Status:** 🎉 Phase 1 schema foundation COMPLETE. 🎉 **Phase 2 Step 1 (Leagues) COMPLETE 2026-05-19** — 2.1a bootstrap (`324f380`) + 2.1b hook/consumption refactor (`2f81b2b`) + 2.1c admin CRUD UI (`96e9951`). 🚨 **Pending action: `firebase deploy --only firestore:rules`** to apply admin-only write gate + unblock useLeagues hook reads (currently silent-falling-back to constants per default-deny). **Next: Phase 2.2 (Players global migration) — brief writing pending.**
...
- **🎯 Phase 2.2 — Players global migration** — Per `MULTI_TENANT_MIGRATION_PLAN.md` Phase 2 Step 2 + § 63.15.3. Hoist workspace players to `/players/` with `pbliId` dedup. Workspace UI updates to query global. Pattern reuses Phase 2.1 shape (bootstrap script + hook + admin UI). **READY FOR CC BRIEF WRITING — next strategic action.**
