...
- **[✅ READY pending GO] § 77 Draw Stage 1.** Branch `feat/draw-stage1-overlay`. DrawingOverlay (render-only, perfect-freehand) + BaseCanvas arbiter `drawMode` branch + DrawToolbar + drawStrokes helpers + MatchPage wiring. Entry chip `✏ Rysuj` LANDSCAPE-only on Match scout per § 77 decision #1 (portrait + portrait-FS = no draw). `point.annotations` Firestore object shape, NATIVE-orientation coords (no mirror at write — Stage 2 mirrors at read). Stage 2 = ScoutedTeam aggregation + heatmap toggle (Pozycje/Strzały/Adnotacje) + per-match filter — separate brief after Stage 1 deploys + Jacek smoke OK. Discovery needed first for current ScoutedTeam heatmap toggle state (old docs claim no toggles — verify in CC env).
