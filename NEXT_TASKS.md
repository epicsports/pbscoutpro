...
- **🧩 Field View shell (Opus — IN PROGRESS).** Unify the canvas shell (rail / collapsed-strip / pinned-semantic / focus) across all 10 field views + layer model + black-box `phaseControl` slot. Boundary `POINT_AS_TIMELINE.md §8`; D4 enum ratified; contract `CC_BRIEF_FIELDVIEW_SHELL_BUILD.md`; mockup `docs/mockups/fieldview_matrix.html`; glue rules `FIELD_VIEW_GLUE_ASSESSMENT.md`. **Folds in (NOT separate tickets):** ITEM-2 · ScoutedTeam-vs-Match phase-unify · canvas-unify residual (Ballistics, Phase C).
  ...
  - ✅ **ScoutedTeam wired + smoked** (`d5a67999` + smoke fixes: Rysuj-beside-phase, rail-expand). 👍 from Jacek.
  - ✅ **Match-review wired** — §B phase row → floating FieldPhaseControl; per-team A|B layers (GAP F); End match floats (danger). LIVE for smoke. Full suite 77/0 + 27/27 pixel-diff.
  - ✅ **PlayerStats + Hitability already shell-compliant** (no wireable controls / tabs-based) — the rail-native family of 4 is now consistent. **Mechanical wiring DONE.**
  - 🟥 **DECISION OWED (Opus archetype call) — Phase C resolution** (`FIELD_VIEW_GLUE_ASSESSMENT.md §3-§4`): the remaining views are NOT clean wires.
    - **LayoutDetail + Tactic** use the §76 IMMERSIVE landscape archetype (not the rail). Migrating = replacing immersive with rail+collapse. Opus: is the rail meant to supplant §76 immersive for EDITOR views, or is immersive a valid sibling archetype?
    - **BunkerEditor + Ballistics** — plain pages, no rail content → thin/empty-rail wraps (low value; layers would be BUILT, not wired). Ballistics carries the FieldCanvas→InteractiveCanvas residual.
    - **TrainingResults** — a results dashboard, NOT a rail view (drop from the set).
    - **Scout-point (capture)** — own structural-migration brief (not on the rail; most fragile). `§2`.
  - ⏸ Phase C PAUSED pending the archetype ruling — not mechanical CC work.
