...
> **data-quality follow-up (NOT security)** — KIOSK shot writes attribute `scoutedBy = activePlayer.linkedUid` (`KioskLobbyOverlay.jsx:193`), which is the linked-uid of the tapping player, NOT the actual writer (the scout/coach running the KIOSK device). Doesn't affect security (the carve-out for shots uses `isScout`), but misleads any future attribution-driven analytics. Tag for the **data-trust / attribution** workstream (alongside B8).
