...
**Last updated:** 2026-05-24 by CC (**§ 76 Full-screen Stage 1** — IMPL READY on branch `feat/fullscreen-stage1-immersive` (awaiting Jacek GO to merge + deploy). Per § 75 sequencing step 2 (regression FIRST, then full-screen). `useLandscapeMode` extended with `fsActive` + `immersive = isLandscape || fsActive` + `setFullscreen`; new shared `<FullscreenToggle>` (Lucide Maximize2/Minimize2, portrait-only, 44px, amber on active). Match scout + Tactic swap `isLandscape` → `immersive` at 6+5 chrome-hide/fit sites; `<FullscreenToggle>` mounted in canvas frame. Landscape behavior byte-for-byte unchanged. Bundle: main index.js unchanged, MatchPage +0.12 kB, vendor-react +0.73 kB (Lucide icons). Build clean 6.04s, precommit pass, § 27 PASS. Fast-follow ticket (separate): same pattern for ScoutedTeam heatmap / LayoutDetail / BunkerEditor / LayoutAnalytics. NEXT after Jacek GO: DrawingOverlay (clickable toolbar mockup → impl, perfect-freehand-based, iPad/PencilKit arbitration). Earlier today: § 75 (Canvas Interaction Model) `686b73d3`, InteractiveCanvas tap/drag regression fix `6f7158f7` (BaseCanvas wrapped setters), Training guest squad-persist fix `909e7105`, Canvas Step #5 `cb28a26a` (HeatmapCanvas → BaseCanvas + FieldView deprecation), Step #4 `2b6a473` (FieldCanvas → InteractiveCanvas). Smoke owed: § 76 5-step from DEPLOY_LOG (incl. tap-player/drag-player in portrait-FS regression check). Phase 3.c.2 Stage 7.4 super_admin still owed.)
