...
- **Stage 4 — Manual override UI** [queued] — review / reassign low-confidence matches. **Last task in Track C.**
...
- **§ 72 follow-up — `TeamDetailPage` quick add/remove-player buttons** still use single-team `changePlayerTeam` (move, not append). Multi-team management is via the `PlayerEditModal` teams editor; converting the quick-buttons to `teams[]` append/remove is a follow-up. Also deferred: a "+N more teams" header badge on player displays.
