...
| 2 | **Phase 2.2.d + 2.3.d cleanup — drop workspace player/team dual-writes + collapse § 85 backstop** | **Stage 1 ✅ SHIPPED** `33b0d453`; Stages 2-3 await Opus brief | **Stage 1 (reader foundation) SHIPPED 2026-05-28** (`33b0d453`): merged catalog∪workspace readers (`usePlayers`/`useTeams`, dedup + §90 class preference) + `findPlayerByPbliId`→global + 42 ws-only pbliId backfill (global 3200→3242, ws-only 0). **Stages 2-3 remain:** Stage 2 = writers route by pbliId + tighten global `/players` `/teams` create/update to **super_admin-only** (closes the catalog write-leak structurally); Stage 3 = migration (relocate no-pbliId → `/workspaces/{ws}/`, drop pbliId workspace twins; teams half needs the § 90.7 Stage 2 `breakoutVariants` relocation first). Pre-prod context (only Jacek + scout test) makes backwards-compat unnecessary. Drop dual-write from `addPlayer`/`updatePlayer`/`changePlayerTeam`/`addTeam`/`updateTeam` (workspace half), drop `subscribePlayers` (workspace), null-out workspace `linkedUid` backstop (or just delete workspace `/players/` + `/teams/` subcollections entirely). ~30 min, one bundled commit. Touches dataService + ev. workspace player/team subcollection cleanup script. **Catalog data-isolation = same item, not a separate track:** the catalog write-leak (a tenant's `workspace_admin` can create/update global `/players` `/teams` via the `isSuperAdmin() \|\| isWorkspaceAdminOf(ownerWorkspaceId)` rules carve-out, against § 90.9's super_admin-only mandate) closes **structurally** once workspace-local players/teams leave the global collections — the rules-layer carve-out is removed as part of the relocation. (Cross-workspace member/role writes + data-subtree reads are already isolated; only the catalog was not.) |
