...
- **tactics consolidation — OP2 DONE (`--live` 2026-06-17); OP1 OPEN.** ✅ **OP2:** the 24 orphaned `layouts/{id}/tactics` retired — backup → migrate the 5 stranded → delete all 24 (final: orphan 0 · overlay 26 · tournament 9). Code-safe (no reader since `67b95df5`); `scripts/migration/tactics_orphan_cleanup.cjs`. ⏳ **OP1 (Tier-2, code+data, needs Opus brief + GO):** retire the tournament store `tournaments/{tid}/tactics` (9 live) per §97.5 — migrate the 9 + repoint/remove `subscribeTactics`/`addTactic` + update TacticPage tournament route + per-doc target-layout resolution. NOT a pure data migration (has a live reader). Discovery: `docs/architecture/TACTICS_TWO_STORE_DISCOVERY.md` §8.2.
