...
| **B6** | ~~med~~ | Concurrent scouting | "Lazy scout" rotating 4 teams (AvB then CvD on alternate points) doesn't auto-flip side. | **NO-OP — already fixed 2026-04-28** via `teamSideMemoryRef` (`MatchPage.jsx:216`, different shape than briefed `nextPointSide` field but solves both same-team auto-swap + cross-team leak fix without Firestore writes). Cross-team leak fix preserved (savePoint auto-swap LOCAL-ONLY at L1191-1201, no `match.currentHomeSide` write). Per-team memory restores swap intent at URL re-entry. | **✅ DONE 2026-04-28** (closed via diagnosis — no fix needed); smoke owed by Jacek | Jacek 2026-05-12 (SCOUT #5) + Med-batch diagnosis 2026-05-26 |
| **B7** | med | MatchPage layout | Completeness table should move to bottom of scouting view (above END MATCH), collapsed by default. | `CompletenessCard.jsx` mounted at `MatchPage.jsx:1681-1685` (review view, above Points list). No shared foldable component — pattern would be inline `useState + chevron ▶/▼`. End Match isn't standalone CTA (in MoreBtn ActionSheet); "above END MATCH" = bottom of scrollable review content. | **awaiting-decision** (Jacek to confirm: 1. reposition target — after Points list vs sticky vs other; 2. collapse default — collapsed vs expanded; 3. localStorage persist tak/nie). Diagnosis report passed to Opus. | Jacek 2026-05-12 (SCOUT #7) + Med-batch diagnosis 2026-05-26 |
| **B8** | med | ScoutedTeam Strzelanie row | Strzela% formula wrong — denominator should be `N×5 − runners − undeclared` to equal 100%. | TWO surfaces under "Strzela%" name: (1) per-zone table `ScoutedTeamPage.jsx:1057-1086` rendered via `generateInsights.js:774-868` `computeShotTargets` (denom = `points.length`, per-zone frequency, not affected by candidate); (2) computeCompleteness banner `ScoutedTeamPage.jsx:108-183` (denom = `nonRunnerPlayers` = placed-not-runner slots; **algebraically EQUAL to candidate `N×5 − placedRunners − unplaced`** if "undeclared" means unplaced slots). | **🚩 HARD ESCALATE — coaching semantics decision needed from Jacek/Sławek**: what exactly does "MOGLI strzelać" mean? (A) "placed AND not runner" → current is correct, fix = no-op; (B) "placed AND not runner AND has explicit shot decision (not undeclared)" → ~5 LOC change; (C) something else (eliminated-before-shoot, etc.) → bigger semantic shift. Diagnosis report passed to Opus. | Jacek 2026-05-12 (COACH #5) + Med-batch diagnosis 2026-05-26 |
...
| **B22** | low | `workspace.adminUid` ranger1996 | Points to `JDDCmHSQ…` — a deleted Firebase Auth user. Severity theoretical (Auth account deleted; UIDs aren't recycled). Interacts with Phase 3.c.2 `isWorkspaceAdminOf` rule helper, which trusts adminUid. | One-shot `scripts/migration/repoint_adminuid.cjs` committed: resolves Jacek's uid from email at run time, repoints `adminUid`, clears dead-uid tombstone in `userRoles`, stamps `adminTransferredAt`. Idempotent. | **[READY]** — adminUid repoint authorized; one-shot committed, AWAITING Jacek run + BEFORE/AFTER paste | NEXT_TASKS Fragility cluster |
...
- **B22 adminUid repoint** — **[READY]** authorized 2026-05-27; one-shot `scripts/migration/repoint_adminuid.cjs` committed; awaiting Jacek run + BEFORE/AFTER paste.
