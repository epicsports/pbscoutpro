...
- **🧩 Field View shell (Opus, NEW — umbrella)** — unify the canvas shell (rail / collapsed-strip / pinned-semantic / focus) across all 10 field views (4/10 on CanvasRailLayout today) + layer model + a black-box `phaseControl` slot. Boundary vs Point-as-Timeline drawn in `docs/POINT_AS_TIMELINE.md §8` (capture "E" switcher + phase MODEL stay PaT-owned; shell only hosts the slot). D4 phase enum ratified (`POINT_AS_TIMELINE.md` D4). Raw material: `docs/architecture/FIELD_VIEW_INVENTORY.md`. **Folds in (NOT separate tickets):** ITEM-2 · ScoutedTeam-vs-Match phase-unify · canvas-unify residual. Opus draws the mockup (Scout-point + Heatmap, tablet-landscape, existing rail) → Jacek GO.
