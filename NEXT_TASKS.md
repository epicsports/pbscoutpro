...
- **🟢 ACTIVE — Full responsive/landscape WIDE-PASS (CD handoff `_CC_BRIEF_wide_pass.md`, 2026-06-24).** ~15 screens across 5 archetypes get tablet/desktop `*Wide` variants. Source of truth = the handoff `prototype/` (extracted at `Downloads/_cc_wide_extract/_HANDOFF_CC/`; NOT the README — it predates `*Wide`).
  - **§0 two-shells:** phone = tab-shell re-skin; tablet/desktop = `AppShellPremiumWide` (sidebar + master-detail), `isMobile` switch; one-screen-with-modes. **§6 `responsiveWide` allowlist (CRITICAL):** responsive screens own width via `useWide()` — must NOT be 460px-wrapped on wide (→ single-column). **Verify our app's wide dispatch doesn't 460-wrap the listed routes** (1st infra task).
  - **Done (no rework):** Scout/`MatchListPremium` · CSV · Workspace. **Phone done, +Wide needed:** Match/Coach/Opponent/Player-stats/Live/Nav. **Partial→complete:** CoachWide/PlayerWide · My/Team-profile v1 · ranking.
  - **New (bulk):** MyProfileWide · TeamProfileWide · TodayPoints(+Wide) · RosterManageWide · SelectTrainingWide · LayoutsListWide · PointWizardWide · New-player/New-team wizards · OpponentAnalysisWide.
  - **§8 build order (LOCKED):** foundation (ELEV/Crest/RdIcon ✅) → **responsive shell** (gate: all 5 archetypes have a wide variant) → **simple lists** → opponent/live/**field LAST**. Reuse the existing `<Screen>`+Canvas/Rail tier (ONE system, not a 2nd framework). Field block PAUSED behind this.
