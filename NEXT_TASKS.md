...
**Status:** 🎉 Phase 1 COMPLETE. 🎉 Phase 2 Step 1 (Leagues) COMPLETE 2026-05-19. 🎉 Phase 2 Step 2 (Players) effectively COMPLETE 2026-05-19. ✅ Phase 2.3.a Teams bootstrap EXECUTED 2026-05-20 (`a8cb308`). ✅ **Phase 2.3.b useTeams global hook + dual-write + /teams/ rules DEPLOYED 2026-05-20** (`97af95a`, sequenced rules+code deploy verified working). § 63.15.2.X locked policy on externalId canonicality + sister team curation (`732dd8e`). **Next active 🎯: Phase 2.3.c (super admin UI for global teams CRUD with sister team designation per § 63.15.2.X #3 + RANGER/Ranger Warsaw curation). Brief writing pending.**
...
- **🎯 Phase 2.3.c — Super admin UI for global teams CRUD** — READY FOR CC BRIEF WRITING. Pattern reuses Phase 2.1c Leagues + Phase 2.2.c Players. MUST cover BOTH workflows per § 63.15.2.X #4: manual team creation (default for non-NXL tenants, ~40% of teams) AND PBLeagues-imported edit + sister team designation. Hard-delete safety: warn if team is parent (would orphan children) AND warn if any player.teamId references the team (similar to Phase 2.2.c aliasIds safety). Also MUST surface RANGER vs Ranger Warsaw externalId dup for admin resolution (merge or retire one per § 63.15.2.X #7). New `deleteTeamGlobal()` dataService fn (mirror of deletePlayerGlobal from 2.2.c).
