...
- **✅ Crest Krok 2 — "Najczęstsze rozbiegi" SHIPPED `ad679327`** (Jacek "go ahead"): `computeBreakoutRuns` (Break-keyframe position → nearest bunker, count/success/share) + collapsible table in ScoutedTeamPage. Heatmap/timeline/CTA already existed (reused). Gate 129/0.
  - **DATA-HONEST interpretation:** shipped the breakout-TARGET bunker frequency (derivable). The literal wireframe "od→do" lane→bunker is **deferred** (start lane not captured → needs a capture-flow change).
  - **🟡 Smoke owed + flag:** populated table is composition-verified only (e2e fixtures have no breakout positions) — **smoke on a real team with placed positions.** Also: I put the table in the FULL ScoutedTeamPage (not a separate CoachWide master-detail SUMMARY panel as the lo-fi wireframe sketched) — if Jacek wants the summary-panel structure, that's a follow-up.
...
- **🟢 WIDE-PASS — 10 variants SHIPPED autonomously night 2026-06-24/25 (CD handoff `_CC_BRIEF_wide_pass.md`).** Source = handoff `prototype/` (`Downloads/_cc_wide_extract/_HANDOFF_CC/`). Infra was already in place (`useWide`/`useDevice` exist; **no 460-wrap** anti-pattern in our app — §6 moot). All additive · phone byte-identical · build+precommit green · §27 PASS. **Wide path has NO e2e → Jacek tablet/desktop smoke.** Detail in DEPLOY_LOG.
  - **✅ Shipped (10):** `LayoutsListWide` · `RosterManageWide` · `MyProfileWide` · `TeamProfileWide` · `ScoutRankingWide` (empty-state+i18n) · `TodayPointsWide` · `SelectTrainingWide` · `PlayerStatsWide` (+public profile) · `PointWizardWide` (self-log-wizard e2e green) · `CoachTeamListWide`. + `RdIcon` `search` glyph.
  - **🟡 DEFERRED for Jacek (NOT auto-merged — need review):** **New-player + New-team wizards** (behavior change: form→stepped flow, breaks/needs the create-e2e update — Jacek should see the new UX before it ships) · **`OpponentAnalysisWide`** (overlaps the spec'd ScoutedTeamPage landscape master-detail feature — coordinate first to avoid rework; it's §8-LAST anyway).
  - **Already done before tonight:** Scout/`MatchListPremium` · CSV · Workspace · the 🔵 phone screens.
  - **§8 build order (LOCKED):** foundation ✅ → responsive shell ✅ → simple lists ✅ → opponent/live/**field LAST**. Field block remains PAUSED (resume with the scope correction logged above).
...
- **ScoutedTeamPage / OpponentAnalysis (ANALIZA PRZECIWNIKA) — landscape master-detail (FEATURE, Jacek-spec'd 2026-06-24).** Landscape-first; data-critical (heatmap/scope/points).
  - **✅ Sub-stages 1+2 SHIPPED (2026-06-25, `54bba287`):** score bar (match-scoped, real scoreA/scoreB) + match-list master-detail in landscape (row→scope, open-match door kept). Axis/timeline untouched; scoutedteam-rail 6/6.
  - **✅ Sub-stage 3 SHIPPED (2026-06-25, `f0f7afdb`, Jacek GO):** point-axis **phases→single points of the match** — match-scope-only horizontal scrubber, single-point heatmap (guarded no-selection→aggregate), **canonical timeline source reused** (`mapOnePointForTeam` kf#0+`timeline[]` via `pointPhases.js`). **Field-only narrowing confirmed** (tables stay match-aggregate — single-point tables = useless 1-row; axis = show/replay the point on the layout). Aggregate scopes/phase-axis untouched; e2e 8/8.
  - **✅ FULL ScoutedTeamPage feature complete (1+2+3).**
  - **✅ Rozbiegi OSOBOPOZYCJE column + un-truncated names SHIPPED (2026-06-25, `0516a48e`)** — CD brief `_CC_BRIEF_rozbiegi_opponentwide.md` #2. ZAGRAŃ/W-PKT already existed (§60.4); OSOBOPOZYCJE = unique `assignments[i]` (`—` for anonymous scouting, not faked).
  - **✅ OpponentAnalysisWide CONSOLIDATED into ScoutedTeamPage (brief #3)** — no separate component exists in app; ScoutedTeamPage IS the opponent-analysis-wide. m1118 standard already met (rail LEFT [`CanvasRailLayout` default] · layout-first · point-keyframe axis · field-only selection). **🔴 ONE gap flagged:** the **4-layer TEAM A/B × Pos/Shots** toggle — ScoutedTeamPage is **one-side** (scouted team only); a both-teams view needs the **`matchHeatmapPoints(points)` shared-mapper extract from MatchPage** (the long-flagged follow-up below). Jacek decision: does ScoutedTeamPage gain a both-teams heatmap?

- **Field Workspace — unify #7/#10/#11/#12 into ONE screen, 4 modes (CD-designed, 3 decisions LOCKED 2026-06-24; prototype `fielddata.jsx`/`redesign10`/`redesign11` + demo routes `rd-field*`).** One shared FieldSurface + left icon-rail modes: Podgląd(#10) · Przeszkody(#7) · Linie(#11) · Strefy(#11) · Taktyka(#12). Closes 3 extract-flags (RBAC re-entry · hidden click-actions · #12 inconsistency). Built on the already-shared `FieldCanvas` (the clean seam).
  - **Q1 LOCKED — consolidate, keep deep-links:** ONE FieldWorkspace component + mode-state; existing routes become thin entries that PRESET mode/tactic — `/layout/:id/bunkers`→mode=Przeszkody · `/layout/:id/tactic-edit/:tid`→mode=Taktyka + that tactic (**honor `:tacticId`**) · wizard-finish→bunkers auto-nav preserved. ZERO broken entry points.
  - **Q2 LOCKED — obstacle capability SPLIT (not binary `isAdmin`):** position editing = **super_admin** (bunkers = GLOBAL base `/layouts/{id}`, cross-workspace integrity); bunker **naming** = **coach** via per-team overlay (`bunkerNameOverrides`). Przeszkody tab: coach → **rename-only** (positions locked, names editable); super_admin → **full** position editing. Two distinct write targets (overlay vs global base) — keep separated.
  - **Q3 LOCKED — structural primitives PRIMARY:** runner/shot primitives (arrows = runner paths · shot markers) wired to the EXISTING v2 phase fields (`runners`/`shots`/`quickShots`/`zoneShots`) — **NOT a new model**; freehand `annotations` = secondary layer. Both write existing fields.
  - **⏸️ PAUSED at Stage 1 (2026-06-24)** per the wide-pass handoff — CD's §8 build order puts the responsive shell + simple lists BEFORE field+rail. Stage 1 skeleton (`FieldWorkspace` + mode-rail + coach-inclusive RBAC gating) committed on `feat/field-workspace-stage1` (`54913328`), NOT merged. Resume after the wide-pass shell/lists.
  - **🔑 SCOPE CORRECTION (Jacek 2026-06-24) — for when we resume:** there is NO separate layout editor (§4b `LayoutEditorPremium` ≡ `FieldWorkspace`). The **Tactics mode = the scouting-field experience** (place players · assign their shots · pick point phase · draw · pinch/move) **reusing the scouting modules**, redesigned with a **LEFT RAIL** of relevant pieces. NOT a new editor. **Calibration & obstacle NAMING are NOT FieldWorkspace modes** — tactics are made on an already-calibrated/named/zoned/lined field; base-calibration/setup stays in the wizard (super_admin). So drop "Taktyki ↔ Kalibracja" from the mode-rail.
  - **Stage 2 (when resumed):** left icon-rail + the mode panels, reusing the shared `FieldCanvas` interactions read-only (no copy, no new tactic code — scouting untouched by construction). Data-critical → §27 + nets before merge.
...
- **[NIGHT 2026-06-30 dup-audit] TournamentFormModal — merge create+edit.** `NewTournamentModal.jsx` vs the inline `EditTournamentModal` in `MainPage.jsx` (~470-600) are ~80% identical markup (division toggle, layout select, year, validation). Merge into one `<TournamentFormModal mode>`. **RED** (rewires `createTournament`/`updateTournament` write calls) → needs GO. (Other dup-audit RED items already tracked: #3 shared heatmap mapper + #4 unified PointRow = the MatchPage #4 workstream below; #6 player forms = keep-separate per §63.15.3. GREEN #1 ReportTable SHIPPED (`55a1a90a`). GREEN **#2 ScheduleList — PARKED, do NOT merge as-is:** branch `feat/schedule-list` `b0b2a87b` extracted `<ScheduleList>` + migrated CoachTabContent + MatchListPremium, but the fork hit the session limit mid-edit (worktree `agent-a784227f6265c4554` still DIRTY on MatchListPremium) and ran NO build/precommit/e2e/render → **UNVERIFIED**. Resume: review the dirty MatchListPremium edit, finish, then full verify before merge.)
