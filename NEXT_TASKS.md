...
- ⏳ **Phase 2.2.b — usePlayers global + alias resolution + workspace consumption refactor** — CODE COMPLETE 2026-05-19, commit `8614a9b`. **NOT DEPLOYED** — awaiting Jacek sequenced deploy: (1) `firebase deploy --only firestore:rules` first, (2) `npm run deploy` after. 14 files modified (1 hook + 1 dataService dual-write + 1 rules + 11 consumer raw-ID lookup swaps). Centralized usePlayers hook collapsed naive 20-consumer scope to single refactor. Alias resolution via playersById map (canonical + alias keys → canonical doc). See DEPLOY_LOG 2026-05-19 for full smoke test plan.
