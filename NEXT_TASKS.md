...
- **Stage 3 — Granular read + event-scoped aggregation** — PRE-FLIGHT done, D1/D2 decided (§ 70.8). **Build gated on the PPT matcher smoke** — `propagateMatchup` is unverified (0 propagated selfReports ever). Next session: Jacek runs the smoke (training → coach quick-log points → PPT self-log 2 pts via `/player/log` → close matchup) → CC verifies the matched slot/side/position read-only → matcher OK ⇒ build Stage 3 (D1 source pills on ScoutedTeamPage, D2 per-bunker per-event on TrainingResultsPage); matcher wrong ⇒ fix Stage 2 first.
