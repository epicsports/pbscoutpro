...
**Last synced:** 2026-05-25 · main HEAD `<pending-merge>` (§ 85 B2 (c) link-to-global — READY, awaiting GO + 🔴 SEQUENCED DEPLOY rules→migration→code)
...
| **B2 (c)** | ~~high~~ | First-login (new account) | Architectural decision needed: which collection holds the link contract. Workspace-scoped writes/listener failed for any workspace ≠ ranger1996 because picker fed from global `/players/` (Phase 2.2.b). | Fixed via § 85: link ops (self/admin link/unlink + subscribe) migrate to GLOBAL `/players/`. Workspace-scoped self-link carve-out on rules (`isMember(resource.data.ownerWorkspaceId)`) preserves cross-workspace security. Workspace `linkedUid` stays as backstop (Phase 2.2.d cleanup). One-shot migration copies existing workspace `linkedUid` → global. Picker filter at parent level (defense in depth) — admin paths unfiltered. Ownership-transfer invariant preserved (no `ownerWorkspaceId` in self-* diff allowlist). | **✅ FIXED** (awaiting GO + **🔴 sequenced deploy: rules→migration→code**; see § 85). Phase 2.2.d will collapse workspace link-writes + backstop. | Jacek 2026-05-12 (NEW ACCOUNT #1) + High-3 diagnosis ESCALATE |
