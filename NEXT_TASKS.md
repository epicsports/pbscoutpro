...
- ⏳ **Phase 2.3.b — useTeams global hook + workspace consumption refactor + dual-write** — CODE COMPLETE 2026-05-20, commit `97af95a`. **NOT DEPLOYED** — awaiting Jacek sequenced deploy: (1) `firebase deploy --only firestore:rules` first, (2) `npm run deploy` after. 3 files modified (1 hook + 1 dataService dual-write + 1 rules). Read scope collapsed to hook-only refactor (all 20 React consumers use centralized `useTeams` — automatic pickup). Write scope: addTeam + updateTeam dual-write, deleteTeam workspace-only (global delete deferred to 2.3.c). teamsById map added for O(1) parentTeamId resolution. NO helper hooks (useTeam/useChildTeams deferred — `teamsById[id]` covers the use case). breakoutVariants subcollection untouched per § 63.15.2 workspace-context decision. See DEPLOY_LOG 2026-05-20 for 11-step smoke test plan.
