...
**Status:** 🎉 Phase 1 schema foundation COMPLETE 2026-05-19. ✅ Phase 2.1a (Leagues collection bootstrap) DONE 2026-05-19 (commit `324f380`, 3 docs created in `/leagues/`). **Next: Phase 2.1b (workspace consumption refactor — replace theme.js LEAGUES/DIVISIONS reads with Firestore /leagues/ reads in workspace UI) — brief writing pending.**
...
- **🎯 Phase 2.1b — Workspace consumption refactor (read leagues from Firestore)** — App code change. Replace `LEAGUES` + `DIVISIONS` reads from `theme.js` with Firestore `/leagues/` reads in workspace UI (tournament creation flow, team creation flow, settings, anywhere consuming league/division dropdowns). Additive transition pattern — keep `theme.js` constants as fallback if Firestore fails. Reference: § 63.15.1. **READY FOR CC BRIEF WRITING — next strategic action.**
- **Phase 2.1c — Super admin UI for league management** — UX work. Super admin can add/edit leagues + divisions via UI. CRUD on `/leagues/` collection gated to super admin only. Reference: § 63.15.1 access control. Brief TBD after 2.1b stable.
