...
**Status:** 🎉 Phase 1 COMPLETE. 🎉 Phase 2 Step 1 (Leagues) COMPLETE 2026-05-19. 🎉 Phase 2 Step 2 (Players) effectively COMPLETE 2026-05-19. ✅ **Phase 2.3.a Teams bootstrap EXECUTED 2026-05-20** (`a8cb308` scripts + execute run) — 132 global `/teams/` docs created (125 parents + 7 children, 0 orphans, two-pass with parent verification, Option α preserve IDs, verbatim schema). § 63.15.2.X locked policy on externalId canonicality + sister team curation (`732dd8e`). **Next active 🎯: Phase 2.3.b (useTeams global hook + workspace consumption refactor + dual-write — mirrors Phase 2.2.b pattern with addition of useTeam(id) helper for parentTeamId resolution). Brief writing pending.**
...
- **🎯 Phase 2.3.b — useTeams global hook + workspace consumption refactor + dual-write** — READY FOR CC BRIEF WRITING. Mirrors Phase 2.2.b pattern: refactor `useTeams` hook in `src/hooks/useFirestore.js` to read from `/teams/` via onSnapshot, return `teamsById` map for O(1) lookup. Add `useTeam(id)` helper for parent resolution in usePPTIdentity + NewTournamentModal + ScoutTabContent. dataService `addTeam/updateTeam/deleteTeam/breakoutVariants` fns dual-write to global + legacy. firestore.rules `/teams/{teamId}` block (mirror Phase 2.2.b `/players/` pattern). Consumer files: TeamsPage, TeamDetailPage, NewTournamentModal, ScoutTabContent, AttendeesEditor, usePPTIdentity, CSVImport, ScheduleCSVImport, ScheduleImport — most use the centralized `useTeams` hook so changes localize to the hook itself. breakoutVariants subcollection decision: keep workspace-scoped (tactical scout knowledge per § 63.15.2 spec for workspace context) — defer hoist to future phase.
- **Phase 2.3.c — Super admin UI for global teams CRUD** — Brief TBD post 2.3.b stable. Pattern reuses Phase 2.1c Leagues + Phase 2.2.c Players. MUST cover BOTH workflows per § 63.15.2.X #4: manual team creation (default for non-NXL tenants, ~40% of teams) AND PBLeagues-imported edit + sister team designation. Hard-delete safety: warn if team is parent (would orphan children) and warn if any player references via teamId (legacy or via Phase 2.4 TeamMemberships).
- **Phase 2.3.d — Legacy /workspaces/{slug}/teams/ cleanup** — DEFERRED until 2.3.b consumption stable + 2.3.c admin curation complete. Migration script to remove workspace write paths from dataService + drop subcollection.
