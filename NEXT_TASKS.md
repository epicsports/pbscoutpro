...
- **Phase 2.3.d — delete-team global/workspace mismatch** — 🔴 **CONFIRMED LIVE 2026-05-23** on `a1d5bca`. DB scan: global `/teams/` = 299 vs workspace = 298; **1 global orphan** `7rXJ0Z0U3h4wBAaoZzo8` "test team 123123- SKASUJ MNIEEEEEE" (your 2026-05-20 repro). `deleteTeam` (`dataService:318`) hits workspace-only; `useTeams` (`useFirestore:83`) reads global → leak. Pre-existing since Phase 2.3.b (`97af95a`) — NOT a 3.c.2 regression. **Fix surface:** 2 UI callers — `TeamDetailPage:117` + `TeamsPage:66`. **Recommended:** replace both with `retireTeam` (canonical per § 63.15.2.X.1; preserves audit; dual-writes global+workspace correctly already) + clean up the 1 orphan in the same brief. `deleteTeam` stays as super_admin-only hard-delete.
