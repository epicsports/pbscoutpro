...
- **Stage 3 — Granular read + event-scoped aggregation** — **D2 SHIPPED** (matcher verified — § 70.8). `getEventShotFrequencies(trainingId)` + "Break bunkers" per-bunker breakdown on TrainingResultsPage; `selfReports.trainingId` collection-group index deployed. **D1 DEFERRED** — `ScoutedTeamPage` is tournament-scoped (opponent-scouting); § 70 multi-source data is training-scoped → D1 re-scoped as its own brief.
- **Stage 3 D1 (re-spec)** [queued] — source-filtered **training heatmap on TrainingResultsPage** (new surface — the proper home for the scout/coach/player granular view, since § 70 `_meta` data lives in trainings). Pills All·Scout·Coach·Player filtering slots by `_meta[i].source`.
