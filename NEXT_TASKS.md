...
**Last updated:** 2026-05-20 by CC (Phase 3.b super_admin globalRole editing shipped + deployed `bddeb10`; Phase 3.c → 🎯 NEXT; 3.a migration still deferred)
...
- 🎯 **Phase 3.c — Firestore rules refactor (HIGH RISK)** — NEXT. Deps: 3.a code ✅ + 3.b ✅. Implement § 65.3 matrix in rules — per role, per ownership (ownerWorkspaceId checks on /teams/, /players/ writes; per-workspace scoping on tournament/match/scouted/layout/point reads + writes; PII (emails, linkedUid) field-level scoping). Must also cover § 66.3 viewer + player roles (missed by § 65). Most substantial sub-task — enumerate all 30+ role-consuming sites before deploy. Note: ideally run the deferred 3.a `globalRole` migration before 3.c so rules can rely on the field being present on every /users/ doc.
