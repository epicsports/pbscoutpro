...
- **🎯 Phase 3.a — Role schema + migration** ⏳ pending. Add `user.role` + `user.workspaces[]` (with role per workspace) to /users/{uid} schema. Migration script: backfill Jacek = `super_admin`, all others = `coach` in `ranger1996` workspace; change new signup default to `pending_user`. Medium risk. Deps: Phase 2 closure.
