...
- **ScoutedTeamPage / OpponentAnalysis (ANALIZA PRZECIWNIKA) — landscape master-detail (FEATURE, Jacek-spec'd 2026-06-24).** Landscape-first; data-critical (heatmap/scope/points).
  ...
  - **🟡 Sub-stage 3 READY FOR REVIEW (NOT merged — `feat/scoutedteam-point-axis` `24904dc4`):** point-axis **phases→single points of the match** — horizontal scrubber (match-scope only; aggregate scopes keep the phase axis), single-point heatmap = `heatmapPoints.filter(pt.id===selectedPointId)` (guarded: no-selection→all). **Canonical timeline source reused** (`mapOnePointForTeam` from kf#0 `homeData/awayData`+`timeline[]` via `pointPhases.js` — no reinvention, Jacek's reminder honored). build+precommit + e2e **8/8** (scoutedteam-rail/scouted-team-loader/matchreview-rail). **Design calls to confirm:** horizontal scrubber (not the MatchPage split-tap row — that's welded to live-scoring state); **field-only narrowing** (ROZBICIE/STRZELANIE stay match-aggregate — one-line swap if you want them to follow the point too); URL-param `pid`.
