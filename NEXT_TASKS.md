...
- **🐍 Reads Snake — 2nd mini-game + game selector. STAGE 1 docs DONE, ⏸ AWAITING GO for STAGE 2 build.** Brief: `CC_BRIEF — Reads Snake` (chat 2026-06-17); spec `DESIGN_DECISIONS §119`; design ref `docs/prototypes/reads-take-a-break.html`. STEP 0 verified: leaderboard wildcard rule already deployed (no rules step), "Take a Break" entry exists → upgrades to a data-driven **game selector** at `/break` (games move to `/break/reads` + `/break/snake`, own lazy chunks). Reuses §117 Reads Mini scaffolding; `leaderboards/readsSnake`. On GO → STAGE 2 (game + selector + leaderboard + fail-first e2e), merge-on-GO, no iPhone gate.
