...
- **🆕 Player-identity dedup (pbliId primary key)** — SPEC ready: `docs/briefs/CC_BRIEF_PLAYER_DEDUP.md`. Root cause = `CSVImport.jsx` Path 2 creates a 2nd doc when a pbliId row's exact name matches a pbliId-less doc on a *different* team. Policy (Jacek): obvious (one exact-name pbliId-less candidate) → auto-claim; ambiguous → flag a super-admin to reconcile via `MergePlayersModal`. 3 items: import-prevention · reconcile queue · migration. **Needs Jacek GO + the "obvious" threshold ratified** (identity = high blast radius).
