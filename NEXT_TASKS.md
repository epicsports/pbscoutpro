...
**Last updated:** 2026-05-24 by CC (**Training guest squad-persist fix** — SHIPPED + deployed `909e7105`. Two-path data-layer fix so every attendee ends up in `training.squads{}` in Firestore regardless of invite order: Option 1 in `AttendeesEditor` (atomic invite-time placement into smallest existing squad) + Option 2 in `SquadEditor` (persist the existing mount-time auto-distribute via `scheduleSave` when `squadsDiffer(initial, next)`). Closes the "Bez składu" / "unassigned" bucket in `TrainingCoachTab` for invited guests. NO UI surfaces touched. Bundle unchanged. Known limitation tracked below: existing squad-vs-squad matchups have frozen roster snapshots — guests NOT backfilled retroactively. Earlier today: Canvas Step #5 `cb28a26a` (HeatmapCanvas → BaseCanvas + FieldView deprecation). Next active = Track A Step #6 (LayoutAnalyticsPage → AnalyticsCanvas) **OR** Step #11 (landscape coach view feature, § 64.10) **OR** Track B Phase 2.4 (TeamMemberships). Smoke owed: training guest squad-persist (3-step from DEPLOY_LOG), Canvas Step #5 (MatchPage heatmap tab = #1 priority — scouting hot-path), TrainingResultsPage source-filter pills, ScoutedTeamPage collapsed+expanded. Phase 3.c.2 Stage 7.4 super_admin still owed. See `docs/ops/HANDOVER.md` SESSION RESET CHECKPOINT.)
