...
| 2 | **Phase 2.2.d/2.3.d isolation cutover — DEFERRED to production-version push** | **Stage 1 ✅ SHIPPED** `33b0d453` (stable baseline); **Stages 2-3 + 6 DEFERRED** → production push (§ 90.10) | **Stage 1 (reader foundation) SHIPPED 2026-05-28** (`33b0d453`) — **the stable baseline:** merged catalog∪workspace readers (`usePlayers`/`useTeams`, dedup + §90 class preference) + `findPlayerByPbliId`→global + 42 ws-only pbliId backfill (global 3200→3242, ws-only 0). **DEFERRED (Jacek 2026-05-28, § 90.10):** Stages 2 (writers route by pbliId + catalog write-lock), 3 (migration: relocate no-pbliId → `/workspaces/{ws}/`, drop pbliId twins), 6 (team/role off the fat catalog doc → workspace roster) ship as **ONE coherent isolation cutover at the production-version push** — Stage 2's catalog lock is coupled to Stage 6's thin doc (locking catalog writes before team membership leaves the doc would strand coach team-assignment of pbliId players via `PlayerEditModal`/`TeamDetailPage`/`ScoutedTeamPage`). **Accepted interim (single-tenant-moot):** shared global catalog ⇒ cross-tenant read overlap + `workspace_admin` can write the global catalog (`firestore.rules:506`/`:566`); both closed by the cutover, accepted for the end-June FIT pilot. |
