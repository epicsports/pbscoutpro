...
> **Layout globalization — model + governance DECIDED (2026-05-31), READY FOR OPUS BUILD BRIEF.** Discovery (read-only) confirmed a clean base/overlay seam → DESIGN_DECISIONS § 96. Model: global base `/layouts/{id}` (bunkers/fieldImage/geometry — read authed, **write `isSuperAdmin()`**, super_admin-curated) + workspace overlay `/workspaces/{slug}/layoutOverlays/{id}` (zones + tactics/insights subcols + `baseLayoutId`, read `isMember`/write `isCoach`). `useLayouts` merges by id → consumers untouched. **Migration trivial:** 5 layouts (all ranger1996), ids preserved → `tournament.layoutId` stays valid, zero scouting rewrite. Directly dissolves the §95 #3 layout-reuse friction (browse library → add overlay). **rules change touches a global library, NOT a tenant-isolation predicate — but still a `firestore:rules` deploy → CONFIRM gate per Firebase policy.** Next: Opus writes the staged build brief.
...
> **Catalog read-cost #2–4** (post-cutover) — one-shot session-cache for the catalog, workspace-scope the crowdsource collectionGroup queries, paginate the 3,242-doc catalog.
