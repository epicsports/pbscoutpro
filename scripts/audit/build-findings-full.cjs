...
// Flag extractor — shared shape (records carry the 9-check fields).
function flagsFor(r) {
  const f = [];
...
let md = `# Cross-device render audit — FINDINGS_FULL (wave 2: stress data × 5-role matrix)\n\n`;
...
console.log(`FINDINGS_FULL.md — P0:${counts.sev.P0} P1:${counts.sev.P1} P2:${counts.sev.P2} | carried:${counts.nov['carried-from-wave1']} new-stress:${counts.nov['new-on-stress']} role:${counts.nov['new-role-specific']} | disappeared:${disappeared.length}`);
