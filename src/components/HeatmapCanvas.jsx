...
export default function HeatmapCanvas({ fieldImage, points = [], mode = 'positions', rosterPlayers = [] }) {
  ...
  useEffect(() => {
    ...
    const drawBlobs = (dataBySlot, radius) => {
      for (let slot = 0; slot < 5; slot++) {
        ...
        pts.forEach(pos => {
          ...
          ctx.fillStyle = grd; ctx.beginPath(); ctx.arc(pos.x * w, pos.y * h, radius, 0, Math.PI * 2); ctx.fill();
        });
        pts.forEach(pos => {
          ctx.beginPath(); ctx.arc(pos.x * w, pos.y * h, 3, 0, Math.PI * 2);
    ...
    if (mode === 'positions') {
      ...
      points.forEach(pt => { for (let i = 0; i < 5; i++) if (pt.players?.[i]) bySlot[i].push(pt.players[i]); });
      drawBlobs(bySlot, 35);
    } else {
      const bySlot = Array.from({ length: 5 }, () => []);
    ...
    // Legend
    ctx.font = `bold ${TOUCH.fontXs}px ${FONT}`; ctx.textBaseline = 'middle';
    const ly = h - 14;
    for (let i = 0; i < 5; i++) {
      const lx = 12 + i * 70;
      ctx.fillStyle = COLORS.playerColors[i]; ctx.beginPath(); ctx.arc(lx, ly, 5, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = COLORS.text; ctx.textAlign = 'left';
      ctx.fillText((rosterPlayers[i]?.nickname || rosterPlayers[i]?.name || `P${i + 1}`).slice(0, 6), lx + 9, ly);
    }
