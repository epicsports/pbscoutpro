...
- **Read-volume C (rollup lever):** **Stage 1 COMPLETE** — 1.1 per-match rollup emit + backfill (`21019436`) + **1.4 consumers migrated to hybrid-read** (`bec3a038` — coach-heatmap/PlayerStats/LayoutAnalytics now read 1 rollup doc/match; parity-proven). **NEXT: 1.2** layout-aggregate (`/layoutAggregates`, dormant until selfLog) + **Stage 2** raw collectionGroup scoping + carve-outs (folds §90 1.2/1.3; tenant-isolation rules → CONFIRM; **GATE: before FIT selfLog go-live**) + **Stage 3** optional IndexedDB cache. **Edge note:** rollup = match-end snapshot; post-merge point edits aren't reflected until re-merge (low-risk; flag if post-end editing becomes a workflow).
