...
- **Read-volume C (rollup lever):** **Stage 1 COMPLETE** — 1.1 per-match rollup emit + backfill (`21019436`) + **1.4 consumers migrated to hybrid-read** (`bec3a038` — coach-heatmap/PlayerStats/LayoutAnalytics now read 1 rollup doc/match; parity-proven). **1.2 SHIPPED** (`5b9ee781` — `/layoutAggregates` pool + 2 crowdsource consumers migrated + backfill + parity 6/6+7/7; read rule live, write rule = 2.4). **NEXT: Stage 2** — scope raw `selfReports`/`shots` CGs → `isMember(resource.data.workspaceSlug)` + player carve-out (`usePlayerBreakoutHistory`) + `/layoutAggregates` increment-only write rule (all tenant-isolation → CONFIRM; **GATE: before FIT selfLog go-live**) + composite indexes. **Stage 3** optional IndexedDB cache. **Edge note:** rollup = match-end snapshot; post-merge point edits aren't reflected until re-merge (low-risk; flag if post-end editing becomes a workflow).
