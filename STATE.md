...
- **§90 A1** — catalog isolation **cutover**. **Stage 1.1 SHIPPED** (`7fa7e780` — `workspaceSlug` denormalized + backfilled). **Stage 1.2/1.3 (scoped collectionGroup rules) FOLDED into read-volume C** (crowdsource stays cross-tenant via a precomputed aggregate = the rollup; build aggregate → migrate consumers → scope raw; GATE: before FIT multi-tenant selfLog go-live). **🔴 Stage 2 (twin decommission) [ESCALATE]-blocked — needs reshape:** 2.1 audit found live twin reads → (a) `repairScoutedDivisionsForTournament:774` reads twin `teams` (repoint to global first); (b) `breakoutVariants` is LIVE workspace data under the teams-twin path (0 docs today, but its write rule IS the teams-twin rule block — deleting the block/path breaks the feature) → decide exclude-with-scoped-rule / relocate / deprecate. Players twin (2634) clean/deletable; teams twin docs (299) deletable only after (a)+(b). **Stage 3** (`/layouts` cleanup) + Stage 2 deletes gated on Jacek's dated window. **(2 workspaces now: pbfit + ranger1996.)**
