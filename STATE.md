...
**Last reconciled:** 2026-06-05 · main HEAD `d9a020df` (§90 Stage 2B/3 twin decommission EXECUTED — 2,938 twin/layout docs `--live` deleted, teams+layouts rules removed; players block + `deletePlayer` semantics escalated. Earlier same day: claim flow 1b · bunker-naming re-key · B4 · Read-volume C Stage 2).
...
- **§90 A1** — catalog isolation **cutover**. **Stage 1.1 SHIPPED** (`7fa7e780`). **Stage 1.2/1.3 FOLDED into read-volume C — ✅ DONE** (`73aba833`). **Stage 2A ✅ SHIPPED** (`ec5a008c`). **🟡 Stage 2B + 3 EXECUTED 2026-06-05** (`840b2fbc` code + `d9a020df` rules; Jacek CONFIRM): twin-writes dropped (`mirrorTwin` + 4 callsites + retire/unretire/**setParentTeam** direct writes) + app deployed → **`--live` deleted 2,634 player + 299 team twins + 5 `/layouts` stragglers = 2,938 docs** (drift +0, 0 remaining; local PII JSON backups at `…/dk/pbscoutpro_backups_90_2b3/2026-06-05T13-55-25-110Z/`) → removed **teams + layouts** twin rule blocks (global catalog `:614`/`:674`/`:699` + relocated breakoutVariants + selfReports + layoutOverlays intact). **🔴 OPEN — players twin rule block KEPT + escalated:** `deletePlayer` (PlayersPage roster delete) is a LIVE twin writer the 2A read-only audit missed; global delete is `isSuperAdmin()`-only → removing the block breaks the (already-silent-no-op) roster delete. **Jacek decision owed:** roster-delete semantics in the global-catalog model — (a) repoint→`deletePlayerGlobal`+workspace_admin-of-owner delete rule / (b) super_admin-only button / (c) remove it. Then drop the players block + deploy. **(2 workspaces: pbfit + ranger1996.) Dead-code follow-up:** `deleteTeam` + `subscribeLayouts`/`addLayout`/`updateLayout`/`deleteLayout` are now unreachable — removable in a later cleanup.
...
## ✅ Recently shipped
- **2026-06-05 (§90 2B/3):** twin + `/layouts` decommission EXECUTED (`840b2fbc`+`d9a020df`) — 2,938 `--live` docs deleted (2,634 player + 299 team twins + 5 layout stragglers; local PII backups), twin-writes dropped + deployed, teams+layouts twin rule blocks removed. **OPEN:** players block + `deletePlayer` roster-delete semantics (escalated); ghost cleanup dropped (5 not 566; Jacek's own test accounts); §98 zone-field drop deferred (smoke owed).
