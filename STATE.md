...
- **Read-volume C (rollup lever, charter brief landed):** **Stage 1.1 SHIPPED** (`21019436` — per-match rollup `matches/{mid}/rollup/snapshot` emitted at match-end + 246 backfilled + parity-proven; consumers not yet migrated). **NEXT: Stage 1.4** migrate coach-heatmap/PlayerStats/LayoutAnalytics to hybrid-read the rollup (per-consumer parity) + **1.2** layout-aggregate. **Stage 2** = raw collectionGroup scoping + carve-outs (folds §90 1.2/1.3; tenant-isolation rules → CONFIRM; GATE: before FIT selfLog go-live). **Stage 3** optional IndexedDB cache.
