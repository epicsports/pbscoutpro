...
| **Tactical rework — `DrawingCanvas` engine + 4 ekrany** | 🔵 | ⚪❄️ | ARCH | CD brief `_CC_BRIEF_tactical_rework.md` (proj. CD, 2026-07-01) + prototyp `prototype/tactical*.jsx`. Jeden współdzielony silnik rysowania (typowane elementy: player/entry/shot/bounce/stroke/pin, oś faz, podgląd↔edycja+fullscreen) **supersedujący** FieldCenter/SHOTMAP/NOTES/TacticWorkspace/TacticBoard. Ekrany: Taktyki (coach), Analiza pola + Moje notatki (zawodnik); coachowe „Notatki" NIE wdrażać. **ARCH-scale** (pokrywa A-10/A-26/A-27), **dotyka żywego rysowania = ❄️ do po Birmingham + GO.** CD chce reality-pass: hooki danych (gracze/strzały/notatki), źródło kalibracji baz, TAK/NIE supersede. |
