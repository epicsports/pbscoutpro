...
| Scout point landscape — „Rysuj" zasłania nazwy drużyn | 🔵 | 🟢* | CD | Live-bug (Jacek 2026-07-01). Brief CD `scout-point-draw-button-fix.md`. **DONE + MERGED** (`c1997dc7`, GO'd freeze exception): chip Rysuj → prawy-dolny róg pola (top:48→bottom:12, 44px), górne rogi=`sideChip` L/R tożsamości (już istniały). Render-proof @1000 landscape ✅ `screenshots/scout-point-draw-chip-bottomright-1000.png`. Deviacje: fullscreen 2-tool-stack z briefu NIE reintro (usunięty a5c3f7b6). **🟢* = Jacek prod-smoke owed** dla pełnego 🟢. |
| Tactic page (rysowanie po layoutach) | ⚪ | ⚪❄️ | CD | redundancja „Gotowe/Save", i18n, loupe LEFT; tactic-creation redesign (oś faz + Warstwy) |
| **Tactical rework — `DrawingCanvas` engine + Coach Taktyki** | 🔵 | 🟠 | CD/CC | **UNFROZEN + na prodzie (beta) 2026-07-01/02** (Jacek: „nikt z tego nie korzysta"). Silnik `src/components/tactical/DrawingCanvas.jsx` + `TacticsCanvasPage` (`/layout/:id/tactics-canvas`). Real-data via additive `tacticalElements`. Done: precision-shot · bounce/pin/entry (nowe) · warstwy · slot-kolory · **fazy Breakout→Settle→Mid + carry-forward** · compact axis. Decyzje: `docs/briefs/TACTICAL_REWORK_DECISIONS.md`. **Owed:** purple-flash re-test · **portrait layout (u CD)** · B6 inspektor (parked, CD) · A9 pole (CD) · A2/A3/A4 runners/bumpy w silniku. Player ekrany „Analiza pola/Moje notatki" = wymyślone/park. Legacy board nietknięty (direct URL). |
